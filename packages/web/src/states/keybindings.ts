...
export class KeybindingsState extends TextualGameState {
  ...
  onEnter() {
    ...
    this.trackClicks(
      this.htmlElement.querySelector(".resetBindings"),
      this.resetBindings
    )
  ...
  updateKeybindings() {
    ...
    for (const category in KEYMAPPINGS) {
      for (const keybindingId in KEYMAPPINGS[category]) {
        ...
        if (mapped.modifiers?.shift) {
          modifiers += "⇪ "
        }
        if (mapped.modifiers?.alt) {
          modifiers += `${T.global.keys.alt} `
        }
        if (mapped.modifiers?.ctrl) {
        ...
        resetBtn.classList.toggle(
          "disabled",
          mapped.builtin || !overrides[keybindingId]
        )
      }
    }
  }
  ...
  resetBindings() {
    ...
    reset.add(() => {
      ...
      this.dialogs.showInfo(
        T.dialogs.keybindingsResetOk.title,
        T.dialogs.keybindingsResetOk.desc
      )
    })
  }
