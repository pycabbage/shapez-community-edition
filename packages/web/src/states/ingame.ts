...
export class InGameState extends GameState {
  ...
  goBackToMenu() {
    if (
      [enumGameModeIds.puzzleEdit, enumGameModeIds.puzzlePlay].includes(
        this.gameModeId
      )
    ) {
  ...
  saveThenGoToState(stateId, payload) {
    if (
      ...
      this.stage === GAME_LOADING_STATES.destroyed
    ) {
      logger.warn(
  ...
  stage3CreateCore() {
    if (this.switchStage(GAME_LOADING_STATES.s3_createCore)) {
      ...
      this.app.backgroundResourceLoader.resourceStateChangedSignal.add(
        ({ progress }) => {
          this.loadingOverlay.loadingIndicator.innerText =
            T.global.loadingResources.replace(
              "<percentage>",
      ...
      this.app.backgroundResourceLoader.getIngamePromise().then(
        () => {
          if (this.creationPayload.gameModeId?.includes("puzzle")) {
    ...
    if (this.switchStage(GAME_LOADING_STATES.s4_B_resumeGame)) {
      if (!this.core.initExistingGame()) {
        this.onInitializationFailure(
          "Savegame is corrupt and can not be restored."
        )
        return
      }
      this.stage5FirstUpdate()
