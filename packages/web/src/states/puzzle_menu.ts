...
export class PuzzleMenuState extends TextualGameState {
  ...
  selectCategory(category) {
    ...
    const activeCategory = this.htmlElement.querySelector(
    ...
    const categoryElement = this.htmlElement.querySelector(
  ...
  selectRootCategory(rootCategory, category) {
    ...
    const activeCategory = this.htmlElement.querySelector(
      ".active[data-root-category]"
    )
    if (activeCategory) {
      activeCategory.classList.remove("active")
    }

    const newActiveCategory = this.htmlElement.querySelector(
  ...
  renderPuzzles(puzzles) {
    ...
    for (const puzzle of puzzles) {
      ...
      if (
        puzzle.author &&
        !["official", "mine"].includes(this.activeCategory)
      ) {
      ...
      if (
        ...
        const completionPercentage = Math.max(
          ...
          Math.min(
            100,
            Math.round((puzzle.completions / puzzle.downloads) * 100.0)
          )
        )
      ...
      const canvas = definition.generateAsCanvas(
        100 * this.app.getEffectiveUiScale()
      )
  ...
  playPuzzle(puzzleId, nextPuzzles) {
    ...
    this.asyncChannel
      .watch(this.app.clientApi.apiDownloadPuzzle(puzzleId))
      .then(
        (puzzleData) => {
          closeLoading()
  ...
  onEnter(payload) {
    ...
    this.trackClicks(
      this.htmlElement.querySelector("button.createPuzzle"),
      () => this.createNewPuzzle()
    )
    this.trackClicks(this.htmlElement.querySelector("button.loadPuzzle"), () =>
      this.loadPuzzle()
    )
  }
  ...
  loadPuzzle() {
    const shortKeyInput = new FormElementInput({
      ...
      defaultValue: "",
      validator: (val) =>
        ShapeDefinition.isValidShortKey(val) || val.startsWith("/"),
    })
