...
export class MainMenuState extends GameState {
  ...
  renderMainMenu() {
    const buttonContainer = this.htmlElement.querySelector(
  ...
  onPuzzleModeButtonClicked(force = false) {
    const hasUnlockedBlueprints = this.app.savegameMgr
  ...
  onLanguageChooseClicked() {
    const setting = /** @type {EnumSetting} */ (
      this.app.settings.getSettingHandleById("language")
    )

    const { optionSelected } = this.dialogs.showOptionChooser(
      T.settings.labels.language.title,
      {
    ...
    optionSelected.add((value) => {
      ...
      // Update current icon
      this.htmlElement
        .querySelector("button.languageChoose")
        .setAttribute("data-languageIcon", value)
    }, this)
  }
  ...
  renderSavegames() {
    const oldContainer = this.htmlElement.querySelector(
      ".mainContainer .savegames"
    )
    if (oldContainer) {
      oldContainer.remove()
    }
    ...
    if (games.length > 0) {
      ...
      for (let i = 0; i < games.length; ++i) {
        ...
        makeDiv(
          ...
          ["playtime"],
          formatSecondsToTimeAgo(
            (new Date().getTime() - games[i].lastUpdate) / 1000.0
          )
        )
        ...
        this.trackClicks(renameButton, () =>
          this.requestRenameSavegame(games[i])
        )
  ...
  checkForModDifferences(savegame) {
    ...
    const signals = this.dialogs.showWarning(
      T.dialogs.modsDifference.title,
      dialogHtml,
  ...
  onContinueButtonClicked() {
    ...
    if (!savegame) {
      console.warn(
        "No savegame to continue found:",
        this.app.savegameMgr.currentData.savegames
      )
      return
    }
