...
export class PreloadState extends GameState {
  ...
  onEnter() {
    ...
    /** @type {HTMLElement} */
    this.progressElement = this.htmlElement.querySelector(
      "#ll_progressbar span"
    )
    ...
    this.setStatus("Booting")
      ...
      .then(() => {
        ...
        if (this.app.platformWrapper.getSupportsFullscreen()) {
          this.app.platformWrapper.setFullscreen(
            this.app.settings.getIsFullScreen()
          )
        }
      })
      ...
      .then(() => {
        this.app.backgroundResourceLoader.resourceStateChangedSignal.add(
          ({ progress }) => {
            this.setStatus(
      ...
      .then(() => {
        ...
        return this.app.storage
          ...
          .then((version) => {
            ...
            return new /** @type {typeof Promise<void>} */ (Promise)(
              (resolve) => {
                this.dialogs
                  .showInfo(T.dialogs.updateSummary.title, dialogHtml)
                  .ok.add(resolve)
              }
            )
          })
  ...
  update() {
    ...
    if (now - this.lastHintShown > this.nextHintDuration) {
       ...
       */
      this.nextHintDuration = Math.max(
        2500,
        (hintText.length / 650) * 60 * 1000
      )
    }
  }
  ...
  showResetConfirm() {
    if (
      confirm(
        "Are you sure you want to reset the app? This will delete all your savegames!"
      )
    ) {
      this.resetApp()
    }
