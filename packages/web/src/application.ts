...
import { STORAGE_SAVES, Storage } from "./platform/storage"
...
export class Application {
  ...
  async boot() {
    ...
    this.trackedIsRenderable = new TrackedState(
      this.onAppRenderableStateChanged,
      this
    )

    /** @type {TypedTrackedState<boolean>} */
    this.trackedIsPlaying = new TrackedState(
      this.onAppPlayingStateChanged,
      this
    )
  ...
  registerEventListeners() {
    ...
    document.addEventListener(
      "touchstart",
      this.updateFocusAfterUserInteraction.bind(this),
      true
    )
    document.addEventListener(
      "touchend",
  ...
  onFrameEmitted(dt) {
    ...
    const currentState = this.stateMgr.getCurrentState()
    this.trackedIsPlaying.set(currentState?.getIsIngame())
    if (currentState) {
  ...
  getEffectiveUiScale() {
    return (
      this.platformWrapper.getUiScale() * this.settings.getInterfaceScaleValue()
    )
  }
