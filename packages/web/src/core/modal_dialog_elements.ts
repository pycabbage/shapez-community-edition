...
import {
  ClickDetector,
  type ClickDetectorConstructorArgs,
} from "./click_detector"
import { globalConfig } from "./config"
import { InputReceiver, type KeydownEvent } from "./input_receiver"
import { createLogger } from "./logging"
import type { FormElement } from "./modal_dialog_forms"
import { STOP_PROPAGATION, Signal } from "./signal"
...
export class Dialog<T extends string = never, U extends unknown[] = []> {
  ...
  handleKeydown({
    ...
    alt,
    ctrl,
  }: KeydownEvent): undefined | STOP_PROPAGATION {
  ...
  createElement() {
    ...
    if (this.closeButton) {
      ...
      this.trackClicks(
        closeBtn,
        () => this.internalButtonHandler("close-button"),
        {
      ...
      this.inputReceiver.backButton.add(() =>
        this.internalButtonHandler("close-button")
      )
    }
    ...
    if (this.buttonIds.length > 0) {
      ...
      for (let i = 0; i < this.buttonIds.length; ++i) {
        const [buttonId, buttonStyle, rawParams] = this.buttonIds[i].split(
          ":"
        ) as [T, string, string?] // @TODO: some button strings omit `buttonStyle`
        ...
        if (isEscape && this.closeButton) {
          logger.warn(
            "Showing dialog with close button, and additional cancel button"
          )
        }
...
export class DialogOptionChooser extends Dialog<"optionSelected", [string]> {
  ...
  constructor({
    ...
    options.options.forEach(
      ({ value, text, desc = null, iconPrefix = null }) => {
        const descHtml = desc ? `<span class="desc">${desc}</span>` : ""
        const iconHtml = iconPrefix
          ? `<span class="icon icon-${iconPrefix}-${value}"></span>`
          : ""
        html += `
