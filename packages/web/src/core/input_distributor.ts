...
export class InputDistributor {
  ...
  popReceiver(receiver: InputReceiver) {
    if (this.receiverStack.indexOf(receiver) < 0) {
      assert(
        false,
        `Can not pop receiver ${receiver.context}  since its not contained`
      )
      logger.error(
        "Can not pop receiver",
        receiver.context,
        "since its not contained"
      )
      return
    }
  ...
  handleKeyMouseDown(event: KeyboardEvent | MouseEvent) {
    const keyCode =
      event instanceof MouseEvent ? event.button + 1 : event.keyCode
    if (
      keyCode === 4 || // MB4
      keyCode === 5 || // MB5
      keyCode === 9 || // TAB
