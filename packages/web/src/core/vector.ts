...
export class Vector {
  ...
  toStreetSpace() {
    return new Vector(
      Math.floor(this.x / halfTileSize + 0.25),
      Math.floor(this.y / halfTileSize + 0.25)
    )
  }
  ...
  toWorldSpaceCenterOfTile() {
    return new Vector(
      this.x * tileSize + halfTileSize,
      this.y * tileSize + halfTileSize
    )
  }
  ...
  rotateInplaceFastMultipleOf90(angle) {
    ...
    assert(
      angle >= 0 && angle <= 360,
      `Invalid angle, please clamp first: ${angle}`
    )
