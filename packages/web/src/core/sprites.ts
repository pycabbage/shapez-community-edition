...
export class AtlasSprite extends BaseSprite {
  ...
  draw(context, x, y, w, h) {
    ...
    if (!link) {
      throw new Error(
        `draw: Link for ${this.spriteName} not known: ${ORIGINAL_SPRITE_SCALE} (having ${Object.keys(this.linksByResolution)})`
      )
    }
  ...
  drawCached(parameters, x, y, w = null, h = null, clipping = true) {
    ...
    if (!link) {
      throw new Error(
        `drawCached: Link for ${this.spriteName} at scale ${scale} not known (having ${Object.keys(this.linksByResolution)})`
      )
    }
  ...
  drawCachedWithClipRect(
    ...
    if (!link) {
      throw new Error(
        `drawCachedWithClipRect: Link for ${this.spriteName} at scale ${scale} not known (having ${Object.keys(this.linksByResolution)})`
      )
    }
    ...
    const destX = x + link.packOffsetX * scaleW + clipRect.x * w
    const destY = y + link.packOffsetY * scaleH + clipRect.y * h
    const destW = link.packedW * scaleW * clipRect.w
    const destH = link.packedH * scaleH * clipRect.h

    const srcX = link.packedX + clipRect.x * link.packedW
    const srcY = link.packedY + clipRect.y * link.packedH
    const srcW = link.packedW * clipRect.w
    const srcH = link.packedH * clipRect.h
  ...
  getAsHTML(w, h) {
    ...
    if (!link) {
      throw new Error(
        `getAsHTML: Link for ${this.spriteName} at scale 0.5 not known (having ${Object.keys(this.linksByResolution)})`
      )
    }
