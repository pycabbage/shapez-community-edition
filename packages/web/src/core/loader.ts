...
class LoaderImpl {
  ...
  getSprite(key) {
    ...
    assert(
      sprite instanceof AtlasSprite || sprite === this.spriteNotFoundSprite,
      "Not an atlas sprite"
    )
  ...
  preloadCSSSprite(key, progressHandler) {
    return this.internalPreloadImage(key, progressHandler).then((image) => {
      if (key.indexOf("game_misc") >= 0) {
        ...
        this.sprites.set(
          key,
          new RegularSprite(image, image.width, image.height)
        )
      }
  ...
  preloadAtlas(atlas, progressHandler) {
    return this.internalPreloadImage(
      atlas.getFullSourcePath(),
      progressHandler
    ).then((image) => {
      // @ts-ignore
