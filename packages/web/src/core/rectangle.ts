...
export class Rectangle {
  ...
  intersectsFully(rect) {
    ...
    return (
      intersection &&
      Math.abs(intersection.w * intersection.h - rect.w * rect.h) < 0.001
    )
  }
   ...
   */
  toCompareableString() {
    return `${round2Digits(this.x)}/${round2Digits(this.y)}/${round2Digits(this.w)}/${round2Digits(this.h)}`
  }
   ...
   */
  toString() {
    return `[x:${round2Digits(this.x)}| y:${round2Digits(this.y)}| w:${round2Digits(this.w)}| h:${round2Digits(this.h)}]`
  }
