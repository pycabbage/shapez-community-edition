...
const INGAME_ASSETS = {
  ...
  sounds: [
    ...
    ...Array.from(Object.values(SOUNDS)).filter(
      (sound) => !MAIN_MENU_ASSETS.sounds.includes(sound)
    ),
  ],
  atlas: atlasFiles,
...
MAIN_MENU_ASSETS.sounds = [
  ...Array.from(Object.values(MUSIC)),
  ...Array.from(Object.values(SOUNDS)),
]
  ...
  async loadAssets({ sprites, sounds, atlas, css }) {
    ...
    for (let i = 0; i < css.length; ++i) {
      promiseFunctions.push((progress) =>
        timeoutPromise(
          this.internalPreloadCss(css[i], progress),
    ...
    for (let i = 0; i < atlas.length; ++i) {
      promiseFunctions.push((progress) =>
        timeoutPromise(
          Loader.preloadAtlas(atlas[i], progress),
          LOADER_TIMEOUT_PER_RESOURCE
        ).catch((err) => {
          logger.error("Failed to load atlas:", atlas[i].sourceFileName, err)
          throw new Error(
            `Atlas ${atlas[i].sourceFileName} failed to load: ${err}`
          )
        })
      )
    }
    ...
    for (let i = 0; i < sounds.length; ++i) {
      promiseFunctions.push((progress) =>
        timeoutPromise(
          ...
          logger.warn(
            "Failed to load sound, will not be available:",
            sounds[i],
            err
          )
        })
