...
export const clickDetectorGlobals = {
  lastTouchTime: -1000,
}
...
export class ClickDetector {
  ...
  internalOnPointerDown(event) {
    ...
    const position = /** @type {typeof ClickDetector} */ (
      this.constructor
    ...
    this.touchstartSimple.dispatch(
      this.clickDownPosition.x,
      this.clickDownPosition.y
    )
  ...
  internalOnPointerMove(event) {
    ...
    const pos = /** @type {typeof ClickDetector} */ (
      this.constructor
    ).extractPointerPosition(event)
  ...
  internalOnPointerEnd(event) {
    ...
    if (this.clickDownPosition) {
      ...
      if (!IS_MOBILE || distance <= this.maxDistance) {
        ...
        console.warn(
          "[ClickDetector] Touch does not count as click:",
          "(was",
          distance,
          ")"
        )
