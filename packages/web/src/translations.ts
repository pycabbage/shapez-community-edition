...
if (G_IS_DEV && globalConfig.debug.testTranslations) {
  ...
  function mapTranslations<T extends Record<string, string | unknown>>(obj: T) {
    for (const key in obj) {
      const value = obj[key]
      if (typeof value === "string") {
        obj[key] = value.replace(/[a-z]/gi, "x") as T[Extract<keyof T, string>]
      } else if (typeof value === "object" && value !== null) {
        mapTranslations(value as Record<string, unknown>)
      }
    }
  }
  mapTranslations(T)
...
>(dest: D | null, src: S | null, addNewKeys = false) {
  ...
  for (const key in dest) {
    if (src[key]) {
      ...
      if (typeof data === "object") {
        matchDataRecursive(dest[key] as D, src[key], addNewKeys)
      } else if (typeof data === "string" || typeof data === "number") {
        // console.log("match string", key);
        dest[key] = src[key] as D[Extract<keyof D, string>]
      } else {
