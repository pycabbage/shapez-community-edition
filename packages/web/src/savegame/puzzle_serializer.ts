...
export class PuzzleSerializer {
  ...
  generateDumpFromGameRoot(root) {
     ...
     */
    const buildings = []
    for (const entity of root.entityMgr.getAllWithComponent(
      ...
      if (signalComp) {
        assert(
          ["shape", "color"].includes(signalComp.signal.getItemType()),
          "not a shape signal"
        )
        buildings.push({
          type: "emitter",
      ...
      if (
        staticComp.getMetaBuilding().id ===
        gMetaBuildingRegistry.findByClass(MetaBlockBuilding).id
      ) {
        buildings.push({
          type: "block",
          pos: {
      ...
      switch (building.type) {
        case "emitter": {
          ...
          const entity = root.logic.tryPlaceBuilding({
            ...
            building: gMetaBuildingRegistry.findByClass(
