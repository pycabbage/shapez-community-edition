...
export class SavegameSerializer {
  ...
  generateDumpFromGameRoot(root, sanityChecks = true) {
    ...
    const data = {
      ...
      pinnedShapes: root.hud.parts.pinnedShapes
        ? root.hud.parts.pinnedShapes.serialize()
        : null,
      waypoints: root.hud.parts.waypoints
  ...
  verifyLogicalErrors(savegame) {
    ...
    for (let i = 0; i < savegame.entities.length; ++i) {
      ...
      for (const componentId in components) {
        ...
        const componentVerifyError = /** @type {StaticComponent} */ (
          componentClass
        ).verify(componentData)
  ...
  deserialize(savegame, root) {
    ...
    errorReason =
      errorReason || root.hubGoals.deserialize(savegame.hubGoals, root)
    errorReason =
      errorReason ||
      this.internal.deserializeEntityArray(root, savegame.entities)
    errorReason =
      errorReason ||
      root.systemMgr.systems.belt.deserializePaths(savegame.beltPaths)

    if (root.hud.parts.pinnedShapes) {
      errorReason =
        errorReason ||
        root.hud.parts.pinnedShapes.deserialize(savegame.pinnedShapes)
    }

    if (root.hud.parts.waypoints) {
      errorReason =
        errorReason || root.hud.parts.waypoints.deserialize(savegame.waypoints)
    }
