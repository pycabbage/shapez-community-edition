...
export class BaseDataType {
  ...
  deserializeWithVerify(value, targetObject, targetKey, root) {
    ...
    if (errorCode) {
      return `serialization verify failed: ${errorCode} [value ${(JSON.stringify(value) || "").substr(0, 100)}]`
    }
...
export class TypeInteger extends BaseDataType {
  serialize(value) {
    assert(
      ...
      `Type integer got non integer for serialize: ${value}`
    )
    return value
  }
...
export class TypePositiveIntegerOrString extends BaseDataType {
  serialize(value) {
    if (Number.isInteger(value)) {
      ...
      assertAlways(
        false,
        `Type integer|string got non integer or string for serialize: ${value}`
      )
    }
    return value
  }
...
export class TypeBoolean extends BaseDataType {
  serialize(value) {
    assert(
      ...
      `Type bool got non bool for serialize: ${value}`
    )
    return value
  }
...
export class TypeString extends BaseDataType {
  serialize(value) {
    assert(
      typeof value === "string",
      `Type string got non string for serialize: ${value}`
    )
    return value
  }
...
export class TypeVector extends BaseDataType {
  serialize(value) {
    assert(
      ...
      `Type vector got non vector for serialize: ${value}`
    )
    return {
...
export class TypeTileVector extends BaseDataType {
  serialize(value) {
    ...
    assert(
      Number.isInteger(value.x) && value.x > 0,
      `Invalid tile x:${value.x}`
    )
    assert(
      Number.isInteger(value.y) && value.y > 0,
      `Invalid tile x:${value.y}`
    )
...
export class TypeNumber extends BaseDataType {
  serialize(value) {
    assert(
      ...
      `Type number got non number for serialize: ${value}`
    )
...
export class TypeClassData extends BaseDataType {
  ...
  deserialize(value, targetObject, targetKey, root) {
    assert(
      false,
      `can not deserialize class data of type ${this.registry.getId()}`
    )
  }
...
export class TypeArray extends BaseDataType {
  ...
  deserialize(value, targetObject, targetKey, root) {
    ...
    const size = this.fixedSize
      ? Math.min(value.length, destination.length)
      : value.length

    for (let i = 0; i < size; ++i) {
      const errorStatus = this.innerType.deserializeWithVerify(
        value[i],
        destination,
        i,
        root
...
export class TypeKeyValueMap extends BaseDataType {
  ...
  deserialize(value, targetObject, targetKey, root) {
    const result = {}
    for (const key in value) {
      const errorCode = this.valueType.deserializeWithVerify(
        value[key],
        result,
        key,
        root
...
export class TypePair extends BaseDataType {
  ...
  constructor(type1, type2) {
    super()
    assert(
      type1 && type1 instanceof BaseDataType,
      "bad first type given for pair"
    )
    assert(
      type2 && type2 instanceof BaseDataType,
      "bad second type given for pair"
    )
    this.type1 = type1
    this.type2 = type2
  }
...
export class TypeStructuredObject extends BaseDataType {
  ...
  deserialize(value, targetObject, targetKey, root) {
    ...
    for (const key in value) {
      ...
      const errorCode = valueType.deserializeWithVerify(
        value[key],
        target,
        key,
        root
