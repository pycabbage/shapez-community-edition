...
function findCode(
  ...
  return getCodeFromBuildingData(
    gMetaBuildingRegistry.findByClass(metaBuilding),
    variant,
    rotationVariant
  )
}
...
export class SavegameInterface_V1006 extends SavegameInterface_V1005 {
  ...
  static computeSpriteMapping() {
    return {
      // Belt
      "sprites/blueprints/belt_top.png": findCode(
        MetaBeltBuilding,
        defaultBuildingVariant,
        0
      ),
      "sprites/blueprints/belt_left.png": findCode(
        MetaBeltBuilding,
        defaultBuildingVariant,
        1
      ),
      "sprites/blueprints/belt_right.png": findCode(
        MetaBeltBuilding,
        defaultBuildingVariant,
        2
      ),
      ...
      "sprites/blueprints/rotator-ccw.png": findCode(
        MetaRotatorBuilding,
        enumRotatorVariants.ccw
      ),
  ...
  static migrate1005to1006(data) {
    ...
    if (dump.hubGoals.currentGoal) {
      dump.hubGoals.currentGoal.required = rebalance(
        dump.hubGoals.currentGoal.required
      )
    }

    const level = Math.min(19, dump.hubGoals.level)
    ...
    for (let i = 0; i < entities.length; ++i) {
      ...
      SavegameInterface_V1006.migrateStaticComp1005to1006(entity)
      ...
      if (components.Unremovable) {
        // @ts-ignore
        components.Unremovable = undefined
      }
      ...
      if (components.ReplaceableMapEntity) {
        // @ts-ignore
        components.ReplaceableMapEntity = undefined
      }
      ...
      if (components.Miner) {
        // @ts-ignore
        components.Miner.chainable = undefined
  ...
  static migrateStaticComp1005to1006(entity) {
    const spriteMapping = SavegameInterface_V1006.computeSpriteMapping()
