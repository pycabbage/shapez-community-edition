...
export class BasicSerializableObject {
  ...
  static getCachedSchema() {
    const id = BasicSerializableObject.getId()

    /* dev:start */
    assert(
      classnamesCache[id] === BasicSerializableObject ||
        classnamesCache[id] === undefined,
      `Class name taken twice: ${id} (from ${BasicSerializableObject.name})`
    )
    classnamesCache[id] = BasicSerializableObject
    /* dev:end */
    ...
    const schema = BasicSerializableObject.getSchema()
  ...
  /** @returns {string|void} */
  static verify(data) {
    return verifySchema(BasicSerializableObject.getCachedSchema(), data)
  }
}
...
export function serializeSchema(obj, schema, mergeWith = {}) {
  for (const key in schema) {
    ...
    if (!schema[key]) {
      assert(
        false,
        `Invalid schema (bad key '${key}'): ${JSON.stringify(schema)}`
      )
    }
...
export function deserializeSchema(
  ...
  for (const key in schema) {
    ...
    const errorStatus = schema[key].deserializeWithVerify(
      data[key],
      obj,
      key,
      obj.root || root
