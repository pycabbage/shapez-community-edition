...
export class MapChunkView extends MapChunk {
  ...
  markDirty() {
    ...
    this.root.map
      .getAggregateForChunk(this.x, this.y, true)
      .markDirty(this.x, this.y)
  }
  ...
  drawForegroundStaticLayer(parameters) {
    ...
    MOD_CHUNK_DRAW_HOOKS.staticBefore.forEach((systemId) =>
      systems[systemId].drawChunk(parameters, this)
    )
    ...
    MOD_CHUNK_DRAW_HOOKS.staticAfter.forEach((systemId) =>
      systems[systemId].drawChunk(parameters, this)
    )
  }
  ...
  static drawSingleWiresOverviewTile({
    ...
    context.fillStyle =
      overrideColor ||
      metaBuilding.getSilhouetteColor(data.variant, data.rotationVariant)
    if (overlayMatrix) {
      for (let dx = 0; dx < 3; ++dx) {
        for (let dy = 0; dy < 3; ++dy) {
          const isFilled = overlayMatrix[dx + dy * 3]
