...
export class MapChunkAggregate {
  ...
  generateOverlayBuffer(canvas, context, w, h, dpi) {
    ...
    for (let x = 0; x < globalConfig.chunkAggregateSize; x++) {
      for (let y = 0; y < globalConfig.chunkAggregateSize; y++) {
        if (
          onlyDirty &&
          !this.dirtyList[globalConfig.chunkAggregateSize * y + x]
        )
          continue
        this.root.map
          .getChunk(
  ...
  drawOverlay(parameters) {
    const aggregateOverlaySize =
      ...
      globalConfig.chunkAggregateSize *
      CHUNK_OVERLAY_RES
    ...
    const dims =
      globalConfig.mapChunkWorldSize * globalConfig.chunkAggregateSize
    ...
    const resourcesScale =
      this.root.app.settings.getAllSettings().mapResourcesScale
    ...
    if (
      ...
      const diameter =
        (70 / parameters.zoomLevel ** 0.35) * (0.2 + 2 * resourcesScale)
