...
export class Entity extends BasicSerializableObject {
  ...
  addComponent(componentInstance, force = false) {
    ...
    assert(
      ...
      "Entity already registered, use EntityManager.addDynamicComponent"
    )
    const id = /** @type {typeof Component} */ (
  ...
  drawDebugOverlays(parameters) {
    ...
    if (G_IS_DEV && staticComp && globalConfig.debug.showAcceptorEjectors) {
      ...
      if (ejectorComp) {
        ...
        for (let i = 0; i < ejectorComp.slots.length; ++i) {
          ...
          drawRotatedSprite({
            ...
            x:
              (slotTile.x + 0.5 + directionVector.x * 0.37) *
              globalConfig.tileSize,
            y:
              (slotTile.y + 0.5 + directionVector.y * 0.37) *
              globalConfig.tileSize,
            angle,
      ...
      if (acceptorComp) {
        const acceptorSprite = Loader.getSprite(
          "sprites/misc/acceptor_slot.png"
        )
