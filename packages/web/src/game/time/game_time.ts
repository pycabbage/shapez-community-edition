...
export class GameTime extends BasicSerializableObject {
  ...
  internalAddDeltaToBudget(deltaMs) {
    ...
    let maxLogicSteps = Math.max(
      3,
      (this.speed.getMaxLogicStepsInQueue() *
        this.root.dynamicTickrate.currentTickRate) /
        60
    )
    ...
    if (
      this.logicTimeBudget >
      this.root.dynamicTickrate.deltaMs * maxLogicSteps
    ) {
  ...
  performTicks(deltaMs, updateMethod) {
    ...
    if (G_IS_DEV && globalConfig.debug.framePausesBetweenTicks) {
      effectiveDelta +=
        globalConfig.debug.framePausesBetweenTicks *
