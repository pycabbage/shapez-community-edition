...
export class GameLogic {
  ...
  checkCanPlaceEntity(entity, { allowReplaceBuildings = true, offset = null }) {
    ...
    if (
      this.root.signals.prePlacementCheck.dispatch(entity, offset) ===
      STOP_PROPAGATION
    ) {
      return false
    }
  ...
  performBulkOperation(operation) {
    ...
    assert(
      !this.root.bulkOperationRunning,
      "Can not run two bulk operations twice"
    )
  ...
  performImmutableOperation(operation) {
    ...
    assert(
      ...
      "Can not run two immutalbe operations twice"
    )
  ...
  computeWireEdgeStatus({ wireVariant, tile, edge }) {
    ...
    const pinEntities = this.root.map.getLayersContentsMultipleXY(
      targetTile.x,
      targetTile.y
    )
    ...
    for (let dx = -1; dx <= 1; ++dx) {
      for (let dy = -1; dy <= 1; ++dy) {
        ...
        if (entity) {
          ...
          for (
            let ejectorSlot = 0;
            ejectorSlot < ejectorSlots.length;
            ++ejectorSlot
          ) {
          ...
          for (
            let acceptorSlot = 0;
            acceptorSlot < acceptorSlots.length;
            ++acceptorSlot
          ) {
