...
export class ItemAcceptorSystem extends GameSystemWithFilter {
  ...
  drawChunk(parameters, chunk) {
    ...
    for (let i = 0; i < contents.length; ++i) {
      ...
      for (
        let animIndex = 0;
        animIndex < acceptorComp.itemConsumptionAnimations.length;
        ++animIndex
      ) {
        ...
        if (
          !chunk.tileSpaceRectangle.containsPoint(realSlotPos.x, realSlotPos.y)
        ) {
        ...
        const fadeOutDirection =
          enumDirectionToVector[staticComp.localDirectionToWorld(direction)]
