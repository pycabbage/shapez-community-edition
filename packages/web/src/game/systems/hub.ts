...
export class HubSystem extends GameSystemWithFilter {
  ...
  redrawHubBaseTexture(canvas, context, w, h, dpi) {
    ...
    if (this.root.hubGoals.isEndOfDemoReached()) {
      ...
      context.fillText(
        T.buildings.hub.endOfDemo.toUpperCase(),
        w / 2,
        h / 2 + 6
      )
    ...
    if (goals.throughputOnly) {
      // Throughput
      const deliveredText =
        T.ingame.statistics.shapesDisplayUnits.second.replace(
          "<shapes>",
    ...
    context.font = "bold 10px GameFont"
    context.fillText(
      T.buildings.hub.deliver.toUpperCase(),
      HUB_SIZE_PIXELS / 2,
      30
    )
    ...
    context.fillText(
      T.buildings.hub.toUnlock.toUpperCase(),
      HUB_SIZE_PIXELS / 2,
      92
    )
