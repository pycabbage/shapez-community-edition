...
export class ItemProcessorSystem extends GameSystemWithFilter {
    ...
    for (let i = 0; i < this.allEntities.length; ++i) {
      ...
      if (currentCharge) {
        ...
        if (
          ...
          !processorComp.queuedEjects.length
        ) {
      ...
      for (let j = 0; j < processorComp.queuedEjects.length; ++j) {
        const { item, requiredSlot, preferredSlot } =
          processorComp.queuedEjects[j]
  ...
  checkRequirements(entity, item, slotIndex) {
    ...
    if (MODS_PROCESSING_REQUIREMENTS[itemProcessorComp.processingRequirement]) {
      return MODS_PROCESSING_REQUIREMENTS[
        itemProcessorComp.processingRequirement
      ].bind(this)({
        entity,
        item,
        slotIndex,
    ...
    switch (itemProcessorComp.processingRequirement) {
      case enumItemProcessorRequirements.painterQuad: {
        ...
        const slotIsEnabled =
          network?.hasValue() && isTruthyItem(network.currentValue)
        if (!slotIsEnabled) {
          return false
        }
        return true
  ...
  canProcess(entity) {
    ...
    switch (processorComp.processingRequirement) {
      ...
      case enumItemProcessorRequirements.painterQuad: {
        ...
        const shapeItem = /** @type {ShapeItem} */ (
          processorComp.inputSlots.get(0)
        )
        if (!shapeItem) {
          return false
        }
        ...
        for (let i = 0; i < 4; ++i) {
          ...
          const networkValue = network?.hasValue() ? network.currentValue : null
      ...
      default:
        assertAlways(
          false,
          `Unknown requirement for ${processorComp.processingRequirement}`
        )
    }
  }
  ...
  startNewCharge(entity) {
    ...
    assert(
      handler,
      `No handler for processor type defined: ${processorComp.type}`
    )
  ...
  process_CUTTER(payload) {
    ...
    for (let i = 0; i < cutDefinitions.length; ++i) {
      ...
      payload.outItems.push({
        item: this.root.shapeDefinitionMgr.getShapeItemFromDefinition(
          definition
        ),
        requiredSlot: i,
      })
  ...
  process_CUTTER_QUAD(payload) {
    ...
    const cutDefinitions =
      this.root.shapeDefinitionMgr.shapeActionCutQuad(inputDefinition)
  ...
  process_ROTATOR(payload) {
    ...
    const rotatedDefinition =
      this.root.shapeDefinitionMgr.shapeActionRotateCW(inputDefinition)
    payload.outItems.push({
      item: this.root.shapeDefinitionMgr.getShapeItemFromDefinition(
        rotatedDefinition
      ),
  ...
  process_ROTATOR_CCW(payload) {
    ...
    const rotatedDefinition =
      this.root.shapeDefinitionMgr.shapeActionRotateCCW(inputDefinition)
    payload.outItems.push({
      item: this.root.shapeDefinitionMgr.getShapeItemFromDefinition(
        rotatedDefinition
      ),
  ...
  process_ROTATOR_180(payload) {
    ...
    const rotatedDefinition =
      this.root.shapeDefinitionMgr.shapeActionRotate180(inputDefinition)
    payload.outItems.push({
      item: this.root.shapeDefinitionMgr.getShapeItemFromDefinition(
        rotatedDefinition
      ),
  ...
  process_STACKER(payload) {
    ...
    assert(
      lowerItem instanceof ShapeItem,
      "Input for lower stack is not a shape"
    )
    assert(
      upperItem instanceof ShapeItem,
      "Input for upper stack is not a shape"
    )
  ...
  process_PAINTER(payload) {
    ...
    const colorizedDefinition =
      this.root.shapeDefinitionMgr.shapeActionPaintWith(
  ...
  process_PAINTER_DOUBLE(payload) {
    ...
    payload.outItems.push({
      item: this.root.shapeDefinitionMgr.getShapeItemFromDefinition(
        colorizedDefinition1
      ),
    })

    payload.outItems.push({
      item: this.root.shapeDefinitionMgr.getShapeItemFromDefinition(
        colorizedDefinition2
      ),
    })
  }
  ...
  process_PAINTER_QUAD(payload) {
    ...
    const colorizedDefinition =
      this.root.shapeDefinitionMgr.shapeActionPaintWith4Colors(
        shapeItem.definition,
