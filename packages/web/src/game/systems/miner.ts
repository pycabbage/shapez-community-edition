...
export class MinerSystem extends GameSystemWithFilter {
  ...
  findChainedMiner(entity) {
    ...
    const staticComp = entity.components.StaticMapEntity
    const contentsBelow = this.root.map.getLowerLayerContentXY(
      staticComp.origin.x,
      staticComp.origin.y
    )
    if (!contentsBelow) {
    ...
    const ejectingDirection = staticComp.localDirectionToWorld(
      ejectingSlot.direction
    )
    ...
    for (let i = 0; i < contents.length; ++i) {
      ...
      if (
        ...
        parameters.context.fillStyle =
          minerComp.cachedMinedItem.getBackgroundColorAsResource()
