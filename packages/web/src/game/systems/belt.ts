...
import { MetaBeltBuilding, arrayBeltVariantToRotation } from "../buildings/belt"
...
export class BeltSystem extends GameSystem {
  constructor(root) {
    ...
    this.root.signals.entityDestroyed.add(
      this.updateSurroundingBeltPlacement,
      this
    )
  ...
  deserializePaths(data) {
    ...
    if (this.beltPaths.length === 0) {
      ...
      logger.warn(
        "Recomputing belt paths (most likely the savegame is old or empty)"
      )
        ...
        for (let i = 0; i < targetEntities.length; ++i) {
          ...
          if (
            ...
            (targetStaticComp.rotation !== rotation ||
              newDirection !== targetBeltComp.direction)
          ) {
  ...
  findFollowUpEntity(entity) {
    ...
    if (followUpEntity) {
      ...
      if (followUpBeltComp) {
        ...
        const acceptedDirection = followUpStatic.localDirectionToWorld(
          enumDirection.top
        )
