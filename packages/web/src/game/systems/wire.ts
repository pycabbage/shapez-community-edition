...
export class WireSystem extends GameSystem {
  ...
  recomputeWiresNetwork() {
    ...
    const tunnelEntities =
      this.root.entityMgr.getAllWithComponent(WireTunnelComponent)
    const pinEntities =
      this.root.entityMgr.getAllWithComponent(WiredPinsComponent)
    ...
    for (let i = 0; i < pinEntities.length; ++i) {
      ...
      for (let k = 0; k < slots.length; ++k) {
        ...
        if (
          slot.type === enumPinSlotType.logicalEjector &&
          !slot.linkedNetwork
        ) {
  ...
  findNetworkForEjector(initialEntity, slot) {
    const currentNetwork = new WireNetwork()
    VERBOSE_WIRES &&
      logger.log(
    ...
    while (entitiesToVisit.length > 0) {
      ...
      if (wireComp) {
        ...
        assert(
          ...
          `Mismatching wire network on wire entity ${wireComp.linkedNetwork ? wireComp.linkedNetwork.uid : "<empty>"} vs ${currentNetwork.uid} @ ${staticComp.origin.toString()}`
        )
        ...
        if (!wireComp.linkedNetwork) {
          if (variantMask && wireComp.variant !== variantMask) {
            ...
            // This one is new! :D
            VERBOSE_WIRES &&
              logger.log("  Visited new wire:", staticComp.origin.toString())
      ...
      if (pinsComp) {
        ...
        assert(
          ...
          `Mismatching wire network on pin slot entity ${slot.linkedNetwork ? slot.linkedNetwork.uid : "<empty>"} vs ${currentNetwork.uid}`
        )
        if (!slot.linkedNetwork) {
          // This one is new
          VERBOSE_WIRES &&
            logger.log("  Visited new slot:", staticComp.origin.toString())
          ...
          newSearchDirections = [
            staticComp.localDirectionToWorld(slot.direction),
          ]
      ...
      if (newSearchTile) {
        ...
        VERBOSE_WIRES &&
          logger.log("   Found", newTargets, "new targets to visit!")
    ...
    if (
      ...
      VERBOSE_WIRES &&
        logger.log("Attached new network with uid", currentNetwork)
    } else {
      ...
      for (let k = 0; k < contents.length; ++k) {
        ...
        if (pinComp) {
          ...
          for (let j = 0; j < pinSlots.length; ++j) {
            ...
            const pinDirection = staticComp.localDirectionToWorld(
              slot.direction
            )
    ...
    for (let y = 0; y < globalConfig.mapChunkSize; ++y) {
      for (let x = 0; x < globalConfig.mapChunkSize; ++x) {
        ...
        if (entity?.components.Wire) {
          ...
          const { opacity, spriteSet } =
            this.getSpriteSetAndOpacityForWire(wireComp)
