...
export class ItemProcessorOverlaysSystem extends GameSystem {
  constructor(root) {
    ...
    this.spriteDisabled = Loader.getSprite(
      "sprites/misc/processor_disabled.png"
    )
    this.spriteDisconnected = Loader.getSprite(
      "sprites/misc/processor_disconnected.png"
    )

    this.readerOverlaySprite = Loader.getSprite(
  ...
  drawChunk(parameters, chunk) {
    ...
    for (let i = 0; i < contents.length; ++i) {
      ...
      if (processorComp) {
        ...
        if (
          !requirement &&
          processorComp.type !== enumItemProcessorTypes.reader
        ) {
          continue
        }
  ...
  drawConnectedSlotRequirement(parameters, entity, { drawIfFalse = true }) {
    ...
    const sprite = anySlotConnected
      ? this.spriteDisabled
      : this.spriteDisconnected
