...
export class DisplaySystem extends GameSystem {
  constructor(root) {
    ...
    for (const colorId in enumColors) {
      ...
      this.displaySprites[colorId] = Loader.getSprite(
  ...
  getDisplayItem(value) {
    ...
    if (MODS_ADDITIONAL_DISPLAY_ITEM_RESOLVER[value.getItemType()]) {
      return MODS_ADDITIONAL_DISPLAY_ITEM_RESOLVER[value.getItemType()].apply(
        this,
        [value]
      )
    }
    ...
    for (let i = 0; i < contents.length; ++i) {
      ...
      if (entity?.components.Display) {
        ...
        if (MODS_ADDITIONAL_DISPLAY_ITEM_DRAW[value.getItemType()]) {
          return MODS_ADDITIONAL_DISPLAY_ITEM_DRAW[value.getItemType()].apply(
            this,
