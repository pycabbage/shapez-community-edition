...
export class StorageSystem extends GameSystemWithFilter {
  constructor(root) {
    ...
    this.storageOverlaySprite = Loader.getSprite(
  ...
  update() {
    for (let i = 0; i < this.allEntities.length; ++i) {
      ...
      const targetAlpha = storageComp.storedCount > 0 ? 1 : 0
      storageComp.overlayOpacity = lerp(
  ...
  drawChunk(parameters, chunk) {
    ...
    for (let i = 0; i < contents.length; ++i) {
      ...
      this.storageOverlaySprite.drawCached(
        parameters,
        center.x - 15,
        center.y + 15,
        30,
      ...
      if (parameters.visibleRect.containsCircle(center.x, center.y + 25, 20)) {
        ...
        context.fillText(
          formatBigNumber(storageComp.storedCount),
          center.x,
          center.y + 25.5
        )
