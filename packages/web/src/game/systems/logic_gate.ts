...
export class LogicGateSystem extends GameSystemWithFilter {
  ...
  update() {
    for (let i = 0; i < this.allEntities.length; ++i) {
      ...
      if (Array.isArray(result)) {
        ...
        assert(
          slotComp.slots[0].type === enumPinSlotType.logicalEjector,
          "Slot 0 should be ejector"
        )
  ...
  compute_NOT(parameters) {
    return isTruthyItem(parameters[0])
      ? BOOL_FALSE_SINGLETON
      : BOOL_TRUE_SINGLETON
  }
  ...
  compute_ROTATE(parameters) {
    ...
    const rotatedDefinitionCW =
      this.root.shapeDefinitionMgr.shapeActionRotateCW(definition)
    return this.root.shapeDefinitionMgr.getShapeItemFromDefinition(
      rotatedDefinitionCW
    )
  }
  ...
  compute_ANALYZE(parameters) {
    ...
    const definition = /** @type {ShapeItem} */ (item).definition
    const lowerLayer = /** @type {import("../shape_definition").ShapeLayer} */ (
      definition.layers[0]
    )
    if (!lowerLayer) {
      return [null, null]
  ...
  compute_STACKER(parameters) {
    ...
    if (
      lowerItem.getItemType() !== "shape" ||
      upperItem.getItemType() !== "shape"
    ) {
      // Bad type
      return null
    }
  ...
  compute_COMPARE(parameters) {
    ...
    switch (itemA.getItemType()) {
      ...
      case "color": {
        return /** @type {ColorItem} */ (itemA).color ===
          /** @type {ColorItem} */ (itemB).color
          ? BOOL_TRUE_SINGLETON
          : BOOL_FALSE_SINGLETON
      }

      case "boolean": {
        return /** @type {BooleanItem} */ (itemA).value ===
          /** @type {BooleanItem} */ (itemB).value
          ? BOOL_TRUE_SINGLETON
          : BOOL_FALSE_SINGLETON
      }
