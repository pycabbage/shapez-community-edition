...
export class UndergroundBeltSystem extends GameSystemWithFilter {
  constructor(root) {
    ...
    this.root.signals.entityManuallyPlaced.add(
      this.onEntityManuallyPlaced,
      this
    )
  ...
  onEntityManuallyPlaced(entity) {
    ...
    if (
      undergroundComp &&
      undergroundComp.mode === enumUndergroundBeltMode.receiver
    ) {
      ...
      for (let i = 0; i < matchingEntrance.range - 1; ++i) {
        ...
        const entityBefore = this.root.map.getTileContent(
          posBefore,
          entity.layer
        )
        const entityAfter = this.root.map.getTileContent(
          currentPos,
          entity.layer
        )
  ...
  findReceiverForSender(entity) {
    ...
    for (
      let searchOffset = 0;
      searchOffset <
      globalConfig.undergroundBeltMaxTilesByTier[undergroundComp.tier];
      ++searchOffset
    ) {
  ...
  handleSender(entity) {
    ...
    if (!cacheEntry) {
      ...
      cacheEntry = undergroundComp.cachedLinkedEntity =
        this.findReceiverForSender(entity)
    }
