...
export class ItemEjectorSystem extends GameSystemWithFilter {
  ...
  recomputeArea(area) {
    ...
    for (let x = 0; x < area.w; ++x) {
      for (let y = 0; y < area.h; ++y) {
        ...
        if (contents?.components.ItemEjector) {
  ...
  recomputeSingleEntityCache(entity) {
    ...
    for (let slotIndex = 0; slotIndex < ejectorComp.slots.length; ++slotIndex) {
      ...
      const ejectSlotWsDirection = staticComp.localDirectionToWorld(
        ejectorSlot.direction
      )
  ...
  tryPassOverItem(item, receiver, slotIndex) {
    ...
    if (itemProcessorComp) {
      ...
      if (
        !this.root.systemMgr.systems.itemProcessor.checkRequirements(
          receiver,
          item,
          slotIndex
        )
    ...
    if (filterComp) {
      ...
      if (
        this.root.systemMgr.systems.filter.tryAcceptItem(
          receiver,
          slotIndex,
          item
        )
    ...
    for (let i = 0; i < contents.length; ++i) {
      ...
      for (let i = 0; i < ejectorComp.slots.length; ++i) {
        ...
        if (nextBeltPath) {
          ...
          const maxProgress =
            (0.5 +
              nextBeltPath.spacingToFirstItem -
              globalConfig.itemSpacingOnBelts) *
            2
        ...
        const tileX =
          realPosition.x + 0.5 + realDirectionVector.x * 0.5 * progress
        const tileY =
          realPosition.y + 0.5 + realDirectionVector.y * 0.5 * progress
