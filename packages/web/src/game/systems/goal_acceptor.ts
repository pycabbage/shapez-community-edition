...
export class GoalAcceptorSystem extends GameSystemWithFilter {
  ...
  update() {
    ...
    for (let i = 0; i < this.allEntities.length; ++i) {
      ...
      if (
        now - goalComp.lastDelivery.time >
        goalComp.getRequiredSecondsPerItem()
      ) {
        goalComp.clearItems()
      }

      if (
        goalComp.currentDeliveredItems < globalConfig.goalAcceptorItemsRequired
      ) {
        allAccepted = false
      }
    }
  ...
  drawChunk(parameters, chunk) {
    ...
    for (let i = 0; i < contents.length; ++i) {
      ...
      const fillPercentage = clamp(
        goalComp.currentDeliveredItems / requiredItems,
        0,
        1
      )
      ...
      if (item) {
        const localOffset = new Vector(0, -1.8).rotateFastMultipleOf90(
          staticComp.rotation
        )
      ...
      goalComp.displayPercentage = lerp(
        goalComp.displayPercentage,
        fillPercentage,
        0.2
      )
