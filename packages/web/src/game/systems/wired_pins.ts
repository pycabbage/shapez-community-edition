...
export class WiredPinsSystem extends GameSystemWithFilter {
  constructor(root) {
    ...
    this.pinSprites = {
      [enumPinSlotType.logicalEjector]: Loader.getSprite(
        "sprites/wires/logical_ejector.png"
      ),
      [enumPinSlotType.logicalAcceptor]: Loader.getSprite(
        "sprites/wires/logical_acceptor.png"
      ),
    }

    this.root.signals.prePlacementCheck.add(this.prePlacementCheck, this)
    this.root.signals.freeEntityAreaBeforeBuild.add(
      this.freeEntityAreaBeforeBuild,
      this
    )
  }
  ...
  checkEntityPinsCollide(entity, offset) {
    ...
    for (let slotIndex = 0; slotIndex < pinsComp.slots.length; ++slotIndex) {
      ...
      const worldPos = entity.components.StaticMapEntity.localTileToWorld(
        slot.pos
      )
      if (offset) {
        worldPos.x += offset.x
      ...
      const collidingEntity = this.root.map.getLayerContentXY(
        worldPos.x,
        worldPos.y,
        "wires"
      )
    ...
    for (let i = 0; i < pinsComp.slots.length; ++i) {
      ...
      if (collidingEntity) {
        ...
        if (!this.root.logic.tryDeleteBuilding(collidingEntity)) {
          assertAlways(
            false,
            "Tried to replace non-repleaceable entity for pins #2"
          )
        }
      }
    }
