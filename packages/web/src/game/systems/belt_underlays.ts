...
export class BeltUnderlaysSystem extends GameSystem {
  ...
  checkIsAcceptorConnected(tile, fromDirection) {
    ...
    if (beltComp) {
      return (
        staticComp.localDirectionToWorld(enumDirection.bottom) === fromDirection
      )
    }
  ...
  checkIsEjectorConnected(tile, toDirection) {
    ...
    if (beltComp) {
      return (
        staticComp.localDirectionToWorld(beltComp.direction) === toDirection
      )
    }
  ...
  computeBeltUnderlayType(entity, underlayTile) {
    ...
    const worldDirection = staticComp.localDirectionToWorld(
      underlayTile.direction
    )
    ...
    for (let i = 0; i < contents.length; ++i) {
      ...
      for (let i = 0; i < underlays.length; ++i) {
        ...
        let { x: x1, y: y1 } = matrix.transformPoint(
          new DOMPoint(
            x - globalConfig.halfTileSize,
            y - globalConfig.halfTileSize
          )
        )
