...
import { SUPPORT_TOUCH, globalConfig } from "../core/config"
...
export class Camera extends BasicSerializableObject {
  constructor(root) {
    ...
    // Handlers
    this.downPreHandler = /** @type {Signal<[Vector, enumMouseButton]>} */ (
      new Signal()
    )
  ...
  addScreenShake(amount) {
    ...
    this.currentShake.x =
      this.currentShake.x + 2 * (Math.random() - 0.5) * scale * amount
    this.currentShake.y =
      this.currentShake.y + 2 * (Math.random() - 0.5) * scale * amount
  }
  ...
  getViewportLeft() {
    return (
      ...
      (this.currentShake.x * 10) / this.zoomLevel
    )
  }
  ...
  internalInitEvents() {
    ...
    if (SUPPORT_TOUCH) {
      this.root.canvas.addEventListener(
        "touchstart",
  ...
  cleanup() {
    ...
    this.root.canvas.removeEventListener(
      "mousedown",
      this.eventListenerMouseDown
    )
    this.root.canvas.removeEventListener(
      "mousemove",
      this.eventListenerMouseMove
    )
  ...
  bindKeys() {
    ...
    mapper
      .getBinding(KEYMAPPINGS.navigation.mapMoveUp)
      .add(() => (this.keyboardForce.y = -1))
    mapper
      .getBinding(KEYMAPPINGS.navigation.mapMoveDown)
      .add(() => (this.keyboardForce.y = 1))
    mapper
      .getBinding(KEYMAPPINGS.navigation.mapMoveRight)
      .add(() => (this.keyboardForce.x = 1))
    mapper
      .getBinding(KEYMAPPINGS.navigation.mapMoveLeft)
      .add(() => (this.keyboardForce.x = -1))
    ...
    mapper
      .getBinding(KEYMAPPINGS.navigation.centerMap)
      .add(() => this.centerOnMap())
  }
  ...
  screenToWorld(screen) {
    const centerSpace = screen.subScalars(
      this.root.gameWidth / 2,
      this.root.gameHeight / 2
    )
  ...
  onMouseWheel(event) {
    ...
    assert(
      ...
      `Got invalid scale in mouse wheel event: ${event.deltaY}`
    )
    assert(
  ...
  onTouchMove(event) {
    ...
    if (event.touches.length === 1) {
      ...
      if (this.currentlyPinching) {
        ...
        const lastDistance = this.lastPinchPositions[0].distance(
          this.lastPinchPositions[1]
        )
        ...
        const oldCenter = this.lastPinchPositions[0].centerPoint(
          this.lastPinchPositions[1]
        )
        ...
        // Compute zoom
        center = center.sub(
          new Vector(this.root.gameWidth / 2, this.root.gameHeight / 2)
        )

        // Apply zoom
        assert(
          Number.isFinite(difference),
          `Invalid pinch difference: ${difference}(last=${lastDistance}, new = ${thisDistance})`
        )
        this.zoomLevel *= difference

        // Stick to pivot point
        const correcture = center
          .multiplyScalar(difference - 1)
  ...
  combinedSingleTouchStartHandler(x, y) {
    ...
    if (
      this.downPreHandler.dispatch(pos, enumMouseButton.left) ===
      STOP_PROPAGATION
    ) {
  ...
  combinedSingleTouchMoveHandler(x, y) {
    ...
    this.didMoveSinceTouchStart =
      this.didMoveSinceTouchStart || delta.length() > 0
  ...
  clampZoomLevel() {
    ...
    assert(
      Number.isFinite(this.zoomLevel),
      `Invalid zoom level *before* clamp: ${this.zoomLevel}`
    )
    ...
    assert(
      Number.isFinite(this.zoomLevel),
      `Invalid zoom level *after* clamp: ${this.zoomLevel}`
    )
  ...
  clampToBounds() {
    ...
    const tileScaleBounds = this.root.gameMode
      .getCameraBounds()
      .allScaled(globalConfig.tileSize)
    this.center.x = clamp(
      this.center.x,
      tileScaleBounds.x,
      tileScaleBounds.x + tileScaleBounds.w
    )
    this.center.y = clamp(
      this.center.y,
      tileScaleBounds.y,
      tileScaleBounds.y + tileScaleBounds.h
    )
  }
  ...
  internalUpdatePanning(now, dt) {
    const baseStrength =
      velocityStrength * this.root.app.platformWrapper.getTouchPanStrength()

    this.touchPostMoveVelocity =
      this.touchPostMoveVelocity.multiplyScalar(velocityFade)
    ...
    if (!this.currentlyMoving && !this.currentlyPinching) {
      ...
      if (len >= velocityMax) {
        this.touchPostMoveVelocity.x =
          (this.touchPostMoveVelocity.x * velocityMax) / len
        this.touchPostMoveVelocity.y =
          (this.touchPostMoveVelocity.y * velocityMax) / len
      }

      this.center = this.center.add(
        this.touchPostMoveVelocity.multiplyScalar(baseStrength)
      )

      // Panning
      this.currentPan = mixVector(this.currentPan, this.desiredPan, 0.06)
      this.center = this.center.add(
        this.currentPan.multiplyScalar((0.5 * dt) / this.zoomLevel)
      )
      this.clampToBounds()
    }
  }
  ...
  internalUpdateZooming(now, dt) {
    if (!this.currentlyPinching && this.desiredZoom !== null) {
      ...
      if (Math.abs(diff) > 0.0001) {
        ...
        assert(
          Number.isFinite(this.desiredZoom),
          `Desired zoom is NaN: ${this.desiredZoom}`
        )
        assert(Number.isFinite(fade), `Zoom fade is NaN: ${fade}`)
        this.zoomLevel = this.zoomLevel * fade + this.desiredZoom * (1 - fade)
        assert(
          Number.isFinite(this.zoomLevel),
          `Zoom level is NaN after fade: ${this.zoomLevel}`
        )
      } else {
  ...
  internalUpdateKeyboardForce(now, dt) {
    if (!this.currentlyMoving && this.desiredCenter == null) {
      ...
      if (
        actionMapper.getBinding(KEYMAPPINGS.navigation.mapMoveRight).pressed
      ) {
        forceX += 1
      }
      ...
      const movementSpeed =
        this.root.app.settings.getMovementSpeed() *
        (actionMapper.getBinding(KEYMAPPINGS.navigation.mapMoveFaster).pressed
          ? 4
          : 1)
