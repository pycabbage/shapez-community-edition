...
export class HubGoals extends BasicSerializableObject {
  ...
  takeShapeByKey(key, amount) {
    assert(
      this.getShapesStoredByKey(key) >= amount,
      `Can not afford: ${key} x ${amount}`
    )
  ...
  getMinerBaseSpeed() {
    if (this.root.gameMode.throughputDoesNotMatter()) {
      return (
        globalConfig.minerSpeedItemsPerSecond * globalConfig.puzzleModeSpeed
      )
    }
    return (
      globalConfig.minerSpeedItemsPerSecond * this.upgradeImprovements.miner
    )
  }
  ...
  getProcessorBaseSpeed(processorType) {
    if (this.root.gameMode.throughputDoesNotMatter()) {
      return (
        globalConfig.beltSpeedItemsPerSecond * globalConfig.puzzleModeSpeed * 10
      )
    }
    ...
    switch (processorType) {
      ...
      case enumItemProcessorTypes.balancer:
        return (
          ...
          this.upgradeImprovements.belt *
          2
        )
      case enumItemProcessorTypes.reader:
        return (
          globalConfig.beltSpeedItemsPerSecond * this.upgradeImprovements.belt
        )
