...
export class Blueprint {
  ...
  static fromUids(root, uids) {
    ...
    for (let i = 0; i < uids.length; ++i) {
      ...
      const pos =
        entity.components.StaticMapEntity.getTileSpaceBounds().getCenter()
    ...
    for (let i = 0; i < uids.length; ++i) {
      newEntities[i].components.StaticMapEntity.origin.subInplace(
        blueprintOrigin
      )
    }
  ...
  getCost() {
    ...
    return findNiceIntegerValue(4 * this.entities.length ** 1.1)
  }
  ...
  draw(parameters, tile) {
    ...
    for (let i = 0; i < this.entities.length; ++i) {
      ...
      if (
        !parameters.root.logic.checkCanPlaceEntity(entity, { offset: tile })
      ) {
  ...
  canAfford(root) {
    ...
    return (
      root.hubGoals.getShapesStoredByKey(
