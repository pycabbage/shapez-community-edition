...
export class DynamicDomAttach {
  ...
  constructor(
    root,
    element,
    { timeToKeepSeconds = 0, attachClass = null, trackHover = false } = {}
  ) {
    /** @type {GameRoot} */
    this.root = root
  ...
  update(isVisible) {
    if (isVisible) {
      ...
      if (this.trackHover && this.root) {
        ...
        if (
          !bounds ||
          this.root.time.realtimeNow() - this.lastComputedBoundsTime > 1.0
        ) {
          bounds = this.lastComputedBounds =
      ...
      if (
        !this.root ||
        this.root.time.realtimeNow() - this.lastVisibleTime >=
          this.timeToKeepSeconds
      ) {
        this.internalDetach()
      }
    }
