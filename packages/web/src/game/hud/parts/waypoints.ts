...
export class HUDWaypoints extends BaseHUDPart {
  ...
  initialize() {
    ...
    this.directionIndicatorSprite = Loader.getSprite(
  ...
  getTextWidth(text) {
    ...
    return (this.cachedTextWidths[text] =
      this.dummyBuffer.measureText(text).width)
  }
  ...
  moveToWaypoint(waypoint) {
    ...
    this.root.camera.setDesiredCenter(
  ...
  requestSaveMarker({ worldPos = null, waypoint = null }) {
    ...
    const markerNameInput = new FormElementInput({
      ...
      validator: (val) =>
        val.length > 0 &&
        (val.length < MAX_LABEL_LENGTH || ShapeDefinition.isValidShortKey(val)),
    })
    const dialog = new DialogWithForm({
      app: this.root.app,
      title: waypoint
        ? T.dialogs.createMarker.titleEdit
        : T.dialogs.createMarker.title,
      desc: fillInLinkIntoTranslation(
        T.dialogs.createMarker.desc,
  ...
  getWaypointScreenParams(waypoint) {
    ...
    if (ShapeDefinition.isValidShortKey(originalLabel)) {
      ...
      item =
        this.root.shapeDefinitionMgr.getShapeItemFromShortKey(originalLabel)
      textWidth = 40
    } else {
  ...
  findCurrentIntersectedWaypoint() {
    ...
    for (let i = 0; i < this.waypoints.length; ++i) {
      ...
      if (params?.screenBounds.containsPoint(mousePos.x, mousePos.y)) {
        return waypoint
      }
    }
  }
  ...
  rerenderWaypointsCompass() {
    ...
    const compassVisible =
      distanceToHub > (10 * globalConfig.tileSize) / this.root.camera.zoomLevel
    const targetCompassAlpha = compassVisible ? 1 : 0

    // Fade the compas in / out
    this.currentCompassOpacity = lerp(
      this.currentCompassOpacity,
      targetCompassAlpha,
      0.08
    )
    ...
    if (iconOpacity > 0.01) {
      ...
      this.waypointSprites.regular.drawCentered(
        context,
        dims / 2,
        dims / 2,
        dims * 0.7
  ...
  drawOverlays(parameters) {
    ...
    this.currentMarkerOpacity = lerp(
      this.currentMarkerOpacity,
      desiredOpacity,
      0.08
    )
    ...
    for (let i = 0; i < this.waypoints.length; ++i) {
      ...
      const isSelected =
        mousePos && bounds.containsPoint(mousePos.x, mousePos.y)
