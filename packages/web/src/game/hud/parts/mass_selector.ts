...
export class HUDMassSelector extends BaseHUDPart {
  ...
  initialize() {
    ...
    this.root.keyMapper
      ...
      .add(this.confirmCut, this)
    this.root.keyMapper
      .getBinding(KEYMAPPINGS.massSelect.massSelectCopy)
      .add(this.startCopy, this)
    this.root.keyMapper
      .getBinding(KEYMAPPINGS.massSelect.massSelectClear)
      .add(this.clearBelts, this)
    ...
    if (this.selectedUids.size > 0) {
      if (
        !this.root.hubGoals.isRewardUnlocked(
          enumHubGoalRewards.reward_blueprints
        )
      ) {
        this.showBlueprintsNotUnlocked()
        return
      }
      this.root.hud.signals.buildingsSelectedForCopy.dispatch(
  ...
  confirmCut() {
    if (
      !this.root.hubGoals.isRewardUnlocked(enumHubGoalRewards.reward_blueprints)
    ) {
      this.showBlueprintsNotUnlocked()
    } else if (
      !this.root.app.settings.getAllSettings().disableCutDeleteWarnings &&
  ...
  onMouseDown(pos, mouseButton) {
    if (
      !this.root.keyMapper.getBinding(KEYMAPPINGS.massSelect.massSelectStart)
        .pressed
    ) {
      return
    }
    ...
    if (
      !this.root.keyMapper.getBinding(
        KEYMAPPINGS.massSelect.massSelectSelectMultiple
      ).pressed
    ) {
      // Start new selection
      this.selectedUids = new Set()
