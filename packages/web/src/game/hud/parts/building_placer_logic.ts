...
export class HUDBuildingPlacerLogic extends BaseHUDPart {
  ...
  initialize() {
     ...
     */
    this.currentMetaBuilding = new TrackedState(
      this.onSelectedMetaBuildingChanged,
      this
    )
     ...
     */
    this.currentVariant = new TrackedState(() =>
  ...
  initializeBindings() {
    ...
    keyActionMapper
      .getBinding(KEYMAPPINGS.placement.rotateWhilePlacing)
      .add(this.tryRotate, this)

    keyActionMapper
      .getBinding(KEYMAPPINGS.placement.rotateToUp)
      .add(this.trySetRotate, this)
    keyActionMapper
      .getBinding(KEYMAPPINGS.placement.rotateToDown)
      .add(this.trySetRotate, this)
    keyActionMapper
      .getBinding(KEYMAPPINGS.placement.rotateToRight)
      .add(this.trySetRotate, this)
    keyActionMapper
      .getBinding(KEYMAPPINGS.placement.rotateToLeft)
      .add(this.trySetRotate, this)
    ...
    keyActionMapper
      .getBinding(KEYMAPPINGS.placement.pipette)
      .add(this.startPipette, this)
    this.root.gameState.inputReceiver.keyup.add(
      this.checkForDirectionLockSwitch,
      this
    )

    // BINDINGS TO GAME EVENTS
    this.root.hud.signals.buildingsSelectedForCopy.add(
      this.abortPlacement,
      this
    )
    ...
    this.root.signals.upgradePurchased.add(() =>
      this.signals.variantChanged.dispatch()
    )
  ...
  get currentBaseRotation() {
    ...
    if (
      metaBuilding &&
      this.preferredBaseRotations.hasOwnProperty(metaBuilding.getId())
    ) {
  ...
  trySetRotate() {
    ...
    if (selectedBuilding) {
      if (
        this.root.keyMapper.getBinding(KEYMAPPINGS.placement.rotateToUp).pressed
      ) {
        this.currentBaseRotation = 0
      } else if (
        this.root.keyMapper.getBinding(KEYMAPPINGS.placement.rotateToDown)
          .pressed
      ) {
        this.currentBaseRotation = 180
      } else if (
        this.root.keyMapper.getBinding(KEYMAPPINGS.placement.rotateToRight)
          .pressed
      ) {
        this.currentBaseRotation = 90
      } else if (
        this.root.keyMapper.getBinding(KEYMAPPINGS.placement.rotateToLeft)
          .pressed
      ) {
  ...
  startPipette() {
    ...
    if (!contents) {
      ...
      if (
        ...
        this.currentMetaBuilding.set(
          gMetaBuildingRegistry.findByClass(MetaMinerBuilding)
        )

        // Select chained miner if available, since that's always desired once unlocked
        if (
          this.root.hubGoals.isRewardUnlocked(
            enumHubGoalRewards.reward_miner_chainable
          )
        ) {
    ...
    if (
      extracted.metaInstance.getId() ===
      gMetaBuildingRegistry.findByClass(MetaHubBuilding).getId()
    ) {
      this.currentMetaBuilding.set(null)
      return
    }
    ...
    if (
      ...
      extracted.metaInstance.getId() ===
        this.currentMetaBuilding.get().getId() &&
  ...
  cycleVariants() {
    ...
    if (!metaBuilding) {
      ...
      const direction = this.root.keyMapper.getBinding(
        KEYMAPPINGS.placement.rotateInverseModifier
      ).pressed
        ? -1
        : 1
  ...
  computeDirectionLockPath() {
    ...
    const currentPos = startTile.copy()
  ...
  onSelectedMetaBuildingChanged(metaBuilding) {
    this.abortDragging()
    this.root.hud.signals.selectedPlacementBuildingChanged.dispatch(
      metaBuilding
    )
    if (metaBuilding) {
      const availableVariants = metaBuilding.getAvailableVariants(this.root)
      ...
      this.fakeEntity.addComponent(
        new StaticMapEntityComponent({
          ...
          rotation: 0,
          tileSize: metaBuilding
            .getDimensions(this.currentVariant.get())
            .copy(),
  ...
  onMouseDown(pos, button) {
    ...
    if (
      ...
      (!metaBuilding ||
        !this.root.app.settings.getAllSettings()
