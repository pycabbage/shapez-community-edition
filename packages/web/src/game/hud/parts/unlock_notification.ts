...
export class HUDUnlockNotification extends BaseHUDPart {
  ...
  createElements(parent) {
    ...
    this.elemSubTitle = makeDiv(
      dialog,
      null,
      ["subTitle"],
      T.ingame.levelCompleteNotification.completed
    )
  ...
  showForLevel(level, reward) {
    ...
    this.elemTitle.innerText =
      T.ingame.levelCompleteNotification.levelTitle.replace(
        "<level>",
        `${level}`.padStart(2, "0")
      )
    ...
    if (gained) {
      gained.forEach(([metaBuildingClass, variant]) => {
        const metaBuilding =
          gMetaBuildingRegistry.findByClass(metaBuildingClass)
