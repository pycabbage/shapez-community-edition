...
export class HUDScreenshotExporter extends BaseHUDPart {
  ...
  initialize() {
    this.root.keyMapper
      .getBinding(KEYMAPPINGS.ingame.exportScreenshot)
      .add(this.startExport, this)
  }
  ...
  doExport() {
    ...
    // Find extends
    const staticEntities = this.root.entityMgr.getAllWithComponent(
    ...
    for (let i = 0; i < staticEntities.length; ++i) {
      const bounds =
        staticEntities[i].components.StaticMapEntity.getTileSpaceBounds()
    ...
    logger.log(
      "Allocating buffer, if the factory grew too big it will crash here"
    )
