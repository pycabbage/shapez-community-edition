...
export class HUDPuzzleEditorReview extends BaseHUDPart {
  ...
  startReview() {
    ...
    setTimeout(() => {
      ...
      const maxTicks =
        this.root.gameMode.getFixedTickrate() *
        globalConfig.puzzleValidationDurationSeconds
      ...
      for (let i = 0; i < maxTicks; ++i) {
        ...
        this.root.time.performTicks(
          deltaMs,
          this.root.gameState.core.boundInternalTick
        )
        simulatedTicks++
  ...
  startSubmit(title = "", shortKey = "") {
    ...
    const items = new Set()
    const acceptors = this.root.entityMgr.getAllWithComponent(
    ...
    while (items.size < 8) {
      // add some randoms
      const item = this.root.hubGoals.computeFreeplayShape(
        Math.round(10 + Math.random() * 10000)
      )
    ...
    const itemInput = new FormElementItemChooser({
      ...
      label: fillInLinkIntoTranslation(
        T.dialogs.submitPuzzle.descIcon,
    ...
    itemInput.valueChosen.add((value) => {
      shapeKeyInput.setValue(
        /** @type {ShapeItem} */ (value).definition.getHash()
      )
    })
  ...
  doSubmitPuzzle(title, shortKey) {
    const serialized = new PuzzleSerializer().generateDumpFromGameRoot(
      this.root
    )
  ...
  validatePuzzle() {
    ...
    const producers = this.root.entityMgr.getAllWithComponent(
    ...
    for (const entity of entities) {
      if (
        this.root.systemMgr.systems.zone.prePlacementCheck(entity) ===
        STOP_PROPAGATION
      ) {
