...
export class HUDPuzzleEditorSettings extends BaseHUDPart {
      ...
      this.root.logic.performImmutableOperation(() => {
        ...
        for (const building of buildings) {
          ...
          for (const key in building.components) {
            building.components[key].copyAdditionalStateTo(
  ...
  updateZoneValues() {
    ...
    this.element.querySelector(".zoneWidth > .value").textContent = String(
      mode.zoneWidth
    )
    this.element.querySelector(".zoneHeight > .value").textContent = String(
      mode.zoneHeight
    )
  }
}
