...
import { makeDiv, round2Digits, round3Digits } from "../../../core/utils"
...
export class HUDDebugInfo extends BaseHUDPart {
  createElements(parent) {
    ...
    this.trackedTickRate = new TrackedState(
      (str) => (tickRateElement.innerText = str)
    )

    const tickDurationElement = makeDiv(this.element, null, ["tickDuration"])
    this.trackedTickDuration = new TrackedState(
      (str) => (tickDurationElement.innerText = str)
    )
    ...
    this.trackedMousePosition = new TrackedState(
      (str) => (mousePositionElement.innerHTML = str)
    )

    const cameraPositionElement = makeDiv(this.element, null, [
      "cameraPosition",
    ])
    this.trackedCameraPosition = new TrackedState(
      (str) => (cameraPositionElement.innerHTML = str)
    )

    this.versionElement = makeDiv(
      this.element,
      null,
      ["version"],
      "version unknown"
  ...
  initialize() {
    ...
    this.root.keyMapper
      .getBinding(KEYMAPPINGS.ingame.toggleFPSInfo)
      .add(() => this.cycleModes())
   ...
   */
  updateLabels() {
    this.trackedTickRate.set(
      `Tickrate: ${this.root.dynamicTickrate.currentTickRate}`
    )
    this.trackedFPS.set(
      `FPS: ${Math.round(this.root.dynamicTickrate.averageFps)} (${round2Digits(1000 / this.root.dynamicTickrate.averageFps)} ms)`
    )
  ...
  updateDetailedInformation() {
    ...
    this.trackedMousePosition.set(
      `Mouse: <code>${mouseTile.x}</code> / <code>${mouseTile.y}</code>`
    )
    this.trackedCameraPosition.set(
      `Camera: <code>${cameraTile.x}</code> / <code>${cameraTile.y}</code>`
    )
  }
