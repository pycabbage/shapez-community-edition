...
export class HUDShop extends BaseHUDPart {
  createElements(parent) {
    ...
    for (const upgradeId in this.root.gameMode.getUpgrades()) {
      ...
      this.trackClicks(handle.buyButton, () =>
        this.tryUnlockNextTier(upgradeId)
      )
  ...
  rerenderFull() {
    for (const upgradeId in this.upgradeToElements) {
      ...
      const currentTierMultiplier =
        this.root.hubGoals.upgradeImprovements[upgradeId]
      ...
      handle.elemDescription.innerText = T.shopUpgrades[upgradeId].description
        ...
        .replace(
          "<newMult>",
          (currentTierMultiplier + tierHandle.improvement).toFixed(2)
        )

      tierHandle.required.forEach(({ shape, amount }) => {
        const container = makeDiv(handle.elemRequirements, null, [
          "requirement",
        ])
  ...
  renderCountsAndStatus() {
    for (const upgradeId in this.upgradeToElements) {
      const handle = this.upgradeToElements[upgradeId]
      for (let i = 0; i < handle.requireIndexToElement.length; ++i) {
        const { progressLabel, progressBar, definition, required } =
  ...
  initialize() {
    ...
    this.keyActionMapper
      ...
      .add(this.close, this)
    this.keyActionMapper
      .getBinding(KEYMAPPINGS.ingame.menuOpenShop)
      .add(this.close, this)
