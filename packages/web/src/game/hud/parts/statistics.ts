...
export class HUDStatistics extends BaseHUDPart {
  createElements(parent) {
    ...
    this.title = makeDiv(
      ...
      null,
      ["title"],
      T.ingame.statistics.title
    )
    this.closeButton = makeDiv(this.title, null, ["closeButton"])
    this.trackClicks(this.closeButton, this.close)

    this.filterHeader = makeDiv(this.dialogInner, null, ["filterHeader"])
    this.sourceExplanation = makeDiv(this.dialogInner, null, [
      "sourceExplanation",
    ])

    this.filtersDataSource = makeDiv(this.filterHeader, null, [
      "filtersDataSource",
    ])
    this.filtersDisplayMode = makeDiv(this.filterHeader, null, [
      "filtersDisplayMode",
    ])
    ...
    const buttonIterateUnit = makeButton(this.filtersDisplayMode, [
      "displayIterateUnit",
    ])
    const buttonDisplaySorted = makeButton(this.filtersDisplayMode, [
      "displaySorted",
    ])
    const buttonDisplayDetailed = makeButton(this.filtersDisplayMode, [
      "displayDetailed",
    ])
    const buttonDisplayIcons = makeButton(this.filtersDisplayMode, [
      "displayIcons",
    ])

    this.trackClicks(buttonIterateUnit, () => this.iterateUnit())
    this.trackClicks(buttonDisplaySorted, () => this.toggleSorted())
    this.trackClicks(buttonDisplayIcons, () =>
      this.setDisplayMode(enumDisplayMode.icons)
    )
    this.trackClicks(buttonDisplayDetailed, () =>
  ...
  setDataSource(source) {
    ...
    this.sourceExplanation.innerText =
      T.ingame.statistics.dataSources[source].description
    if (this.visible) {
      this.rerenderFull()
    }
  }
  ...
  initialize() {
    ...
    this.keyActionMapper
      ...
      .add(this.close, this)
    this.keyActionMapper
      .getBinding(KEYMAPPINGS.ingame.menuClose)
      .add(this.close, this)
    this.keyActionMapper
      .getBinding(KEYMAPPINGS.ingame.menuOpenStats)
      .add(this.close, this)
  ...
  intersectionCallback(entries) {
    for (let i = 0; i < entries.length; ++i) {
      ...
      const handle =
        this.activeHandles[entry.target.getAttribute("data-shape-key")]
      if (handle) {
