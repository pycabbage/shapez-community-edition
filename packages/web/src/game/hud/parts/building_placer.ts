...
export class HUDBuildingPlacer extends HUDBuildingPlacerLogic {
  ...
  createElements(parent) {
    ...
    this.buildingInfoElements.label = makeDiv(
      this.element,
      null,
      ["buildingLabel"],
      "Extract"
    )
    this.buildingInfoElements.desc = makeDiv(
      this.element,
      null,
      ["description"],
      ""
    )
    this.buildingInfoElements.descText = makeDiv(
    ...
    this.buildingInfoElements.hotkey = makeDiv(
      this.buildingInfoElements.desc,
      null,
      ["hotkey"],
      ""
    )
    this.buildingInfoElements.tutorialImage = makeDiv(this.element, null, [
      "buildingImage",
    ])
  ...
  initialize() {
    ...
    this.root.hud.signals.buildingSelectedForPlacement.add(
      this.startSelection,
      this
    )
    ...
    this.variantsAttach = new DynamicDomAttach(
      this.root,
  ...
  rerenderInfoDialog() {
    ...
    this.buildingInfoElements.label.innerHTML =
    ...
    if (rawBinding) {
      ...
      this.buildingInfoElements.hotkey.innerHTML =
        T.ingame.buildingPlacement.hotkeyLabel.replace(
          "<key>",
    ...
    this.buildingInfoElements.tutorialImage.setAttribute(
      "data-icon",
      `building_tutorials/${metaBuilding.getId()}${variant === defaultBuildingVariant ? "" : `-${variant}`}.png`
    )

    removeAllChildren(this.buildingInfoElements.additionalInfo)
    const additionalInfo = metaBuilding.getAdditionalStatistics(
      this.root,
  ...
  rerenderVariants() {
    ...
    for (let i = 0; i < availableVariants.length; ++i) {
      ...
      spriteWrapper.innerHTML = sprite.getAsHTML(
        iconSize * dimensions.x,
        iconSize * dimensions.y
      )
  ...
  drawRegularPlacement(parameters) {
    ...
    if (connectedEntities) {
      for (let i = 0; i < connectedEntities.length; ++i) {
        ...
        const connectedWsPoint =
          connectedEntity.components.StaticMapEntity.getTileSpaceBounds()
            .getCenter()
            .toWorldSpace()
    ...
    const previewSprite = metaBuilding.getBlueprintSprite(
      rotationVariant,
      this.currentVariant.get()
    )
    staticComp.origin = worldPos
  ...
  checkForObstales(from, to, ignorePositions = []) {
    ...
    for (let i = start; i <= end; i++) {
      ...
      if (
        !this.root.logic.checkCanPlaceEntity(this.fakeEntity, {
          allowReplaceBuildings: false,
        })
      ) {
        return true
      }
  ...
  drawDirectionLock(parameters) {
    ...
    const anyObstacle =
      this.checkForObstales(
        this.lastDragTile,
        this.currentDirectionLockCorner,
        [this.lastDragTile, mouseTile]
      ) ||
    ...
    // Draw arrow
    const arrowSprite =
      this.lockIndicatorSprites[anyObstacle ? "error" : this.root.currentLayer]
  ...
  drawMatchingAcceptorsAndEjectors(parameters) {
    ...
    for (let i = 0; i < acceptorSlots.length; ++i) {
      ...
      const sourceTile = acceptorSlotWsTile.add(
        enumDirectionToVector[worldDirection]
      )
      ...
      for (let j = 0; j < sourceEntities.length; ++j) {
        ...
        const ejectorAcceptLocalTile =
          sourceStaticComp.worldToLocalTile(acceptorSlotWsTile)
        ...
        if (sourceEjector?.anySlotEjectsToLocalTile(ejectorAcceptLocalTile)) {
      ...
      drawRotatedSprite({
        ...
        angle: Math.radians(
          enumDirectionToAngle[enumInvertedDirections[worldDirection]]
        ),
        size: 13,
    ...
    for (
      ...
      ejectorSlotIndex < ejectorSlots.length;
      ++ejectorSlotIndex
    ) {
      const slot = ejectorSlots[ejectorSlotIndex]

      const ejectorSlotLocalTile = slot.pos.add(
        enumDirectionToVector[slot.direction]
      )
      const ejectorSlotWsTile =
        staticComp.localTileToWorld(ejectorSlotLocalTile)

      const ejectorSLotWsPos = ejectorSlotWsTile.toWorldSpaceCenterOfTile()
      const ejectorSlotWsDirection = staticComp.localDirectionToWorld(
        slot.direction
      )
      ...
      for (let i = 0; i < destEntities.length; ++i) {
        ...
        const destLocalDir = destStaticComp.worldDirectionToLocal(
          ejectorSlotWsDirection
        )
        if (destAcceptor?.findMatchingSlot(destLocalTile, destLocalDir)) {
          ...
          destLocalDir === enumDirection.top
        ) {
          // Connected to a belt
          isConnected = true
        } else if (minerComp?.chainable && destMiner && destMiner.chainable) {
