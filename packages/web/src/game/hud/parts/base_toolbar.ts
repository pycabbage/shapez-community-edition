...
export class HUDBaseToolbar extends BaseHUDPart {
  ...
  initialize() {
    ...
    for (let i = 0; i < allBuildings.length; ++i) {
      ...
      let rawBinding =
        KEYMAPPINGS.buildings[`${metaBuilding.getId()}_${this.layer}`]
      if (!rawBinding) {
      ...
      itemContainer.setAttribute(
        "data-icon",
        `building_icons/${metaBuilding.getId()}.png`
      )
      ...
      if (
        ...
        !this.inRequiredBuildings(metaBuilding)
      ) {
        ...
        this.trackClicks(
          puzzleLock,
          () => this.toggleBuildingLock(metaBuilding),
          {
            clickSound: null,
          }
        )
    ...
    actionMapper
      .getBinding(KEYMAPPINGS.placement.cycleBuildings)
      .add(this.cycleBuildings, this)
  }
  ...
  update() {
    ...
    if (visible) {
      ...
      for (const buildingId in this.buildingHandles) {
        ...
        const newStatus =
          !handle.puzzleLocked && handle.metaBuilding.getIsUnlocked(this.root)
      ...
      if (recomputeSecondaryToolbarVisibility && this.secondaryDomAttach) {
        ...
        for (let i = 0; i < this.secondaryBuildings.length; ++i) {
          const metaClass = gMetaBuildingRegistry.findByClass(
            this.secondaryBuildings[i]
          )
  ...
  cycleBuildings() {
    ...
    for (let i = 0; i <= this.primaryBuildings.length; ++i) {
      ...
      const metaBuilding = gMetaBuildingRegistry.findByClass(
        this.primaryBuildings[newIndex]
      )
  ...
  toggleBuildingLock(metaBuilding) {
    ...
    if (
      this.inRequiredBuildings(metaBuilding) ||
      !metaBuilding.getIsUnlocked(this.root)
    ) {
      this.root.soundProxy.playUiError()
      return STOP_PROPAGATION
    }
    ...
    for (const entity of entityManager.getAllWithComponent(
    ...
    const currentMetaBuilding =
      this.root.hud.parts.buildingPlacer.currentMetaBuilding
