...
export class HUDMinerHighlight extends BaseHUDPart {
  ...
  draw(parameters) {
    ...
    const throughput = round2Digits(
      connectedEntities.length * this.root.hubGoals.getMinerBaseSpeed()
    )
    ...
    const scale =
      (1 / this.root.camera.zoomLevel) * this.root.app.getEffectiveUiScale()
    ...
    parameters.context.fillText(
      connectedEntities.length === 1
        ...
        : T.ingame.connectedMiners.n_miners.replace(
            "<amount>",
            String(connectedEntities.length)
          ),
      tooltipLocation.x + 10 * scale,
  ...
  findConnectedMiners(entity, seenUids = new Set()) {
    ...
    const connectedMiner =
      this.root.systemMgr.systems.miner.findChainedMiner(entity)
        ...
        if (contents) {
          ...
          if (minerComp?.chainable) {
            ...
            if (!seenUids.has(contents.uid)) {
              if (
                this.root.systemMgr.systems.miner.findChainedMiner(contents) ===
                entity
              ) {
