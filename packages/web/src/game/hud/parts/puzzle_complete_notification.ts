...
export class HUDPuzzleCompleteNotification extends BaseHUDPart {
  ...
  createElements(parent) {
    ...
    this.element = makeDiv(parent, "ingame_HUD_PuzzleCompleteNotification", [
      "noBlur",
    ])

    const dialog = makeDiv(this.element, null, ["dialog"])

    this.elemTitle = makeDiv(
      dialog,
      null,
      ["title"],
      T.ingame.puzzleCompletion.title
    )
  ...
  updateState() {
    this.buttonLikeYes.classList.toggle(
      "active",
      this.userDidLikePuzzle === true
    )
  }
