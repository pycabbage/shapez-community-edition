...
import { Vector, enumDirectionToVector } from "../../../core/vector"
...
export class HUDShapeTooltip extends BaseHUDPart {
  ...
  isActive() {
    ...
    const active =
      ...
      this.root.keyMapper.getBinding(KEYMAPPINGS.ingame.showShapeTooltip)
        .pressed
  ...
  draw(parameters) {
    if (this.isActive()) {
      ...
      if (mousePos) {
        const tile = this.root.camera
          .screenToWorld(mousePos.copy())
          .toTileSpace()
      ...
      for (let i = 0; i < ejectorComp.slots.length; ++i) {
        ...
        const drawPos = staticComp
          .localTileToWorld(slot.pos)
          .toWorldSpaceCenterOfTile()

        slot.lastItem.drawItemCenteredClipped(
          drawPos.x,
          drawPos.y,
          parameters,
          25
