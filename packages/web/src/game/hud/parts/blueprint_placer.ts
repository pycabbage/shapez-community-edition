...
export class HUDBlueprintPlacer extends BaseHUDPart {
  createElements(parent) {
    const blueprintCostShape =
      this.root.shapeDefinitionMgr.getShapeFromShortKey(
        this.root.gameMode.getBlueprintShapeKey()
      )
    const blueprintCostShapeCanvas = blueprintCostShape.generateAsCanvas(80)

    this.costDisplayParent = makeDiv(
      parent,
    ...
    makeDiv(
      this.costDisplayParent,
      null,
      ["label"],
      T.ingame.blueprintPlacer.cost
    )
    const costContainer = makeDiv(
      this.costDisplayParent,
      null,
      ["costContainer"],
      ""
  ...
  initialize() {
    this.root.hud.signals.buildingsSelectedForCopy.add(
      this.createBlueprintFromBuildings,
      this
    )
    ...
    keyActionMapper
      ...
      .add(this.rotateBlueprint, this)
    keyActionMapper
      .getBinding(KEYMAPPINGS.massSelect.pasteLastBlueprint)
      .add(this.pasteBlueprint, this)
  ...
  update() {
    ...
    this.domAttach.update(
      !this.getHasFreeCopyPaste() &&
        currentBlueprint &&
        currentBlueprint.getCost() > 0
    )
    this.trackedCanAfford.set(currentBlueprint?.canAfford(this.root))
  }
