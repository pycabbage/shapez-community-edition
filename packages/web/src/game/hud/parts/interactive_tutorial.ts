...
const tutorialsByLevel = [
    ...
    // 2.1 place a cutter
    {
      ...
      condition: /** @param {GameRoot} root */ (root) =>
        root.entityMgr
          ...
          .filter(
            (e) =>
              e.components.ItemProcessor.type === enumItemProcessorTypes.cutter
          ).length === 0,
    },
    // 2.2 place trash
    {
      ...
      condition: /** @param {GameRoot} root */ (root) =>
        root.entityMgr
          ...
          .filter(
            (e) =>
              e.components.ItemProcessor.type === enumItemProcessorTypes.trash
          ).length === 0,
    },
  ...
  [
    // 21.1 place quad painter
    {
      ...
      condition: /** @param {GameRoot} root */ (root) =>
        root.entityMgr
          ...
          .filter(
            (e) =>
              e.components.ItemProcessor.type ===
              enumItemProcessorTypes.painterQuad
          ).length === 0,
    },
    ...
    // 21.4 activate button
    {
      ...
      condition: /** @param {GameRoot} root */ (root) =>
        root.entityMgr
          .getAllWithComponent(LeverComponent)
...
export class HUDInteractiveTutorial extends BaseHUDPart {
  ...
  onHintChanged(hintId) {
    this.elementDescription.innerHTML =
      T.ingame.interactiveTutorial.hints[hintId]
    document.documentElement.setAttribute("data-tutorial-step", hintId)

    this.elementGif.style.backgroundImage = `url('res/ui/interactive_tutorial.noinline/${hintId}.gif')`
  ...
  draw(parameters) {
    ...
    const currentBuilding =
      this.root.hud.parts.buildingPlacer.currentMetaBuilding.get()

    if (["1_1_extractor"].includes(this.currentHintId.get())) {
      if (
        currentBuilding &&
        currentBuilding.getId() ===
          gMetaBuildingRegistry.findByClass(MetaMinerBuilding).getId()
      ) {
    ...
    if (this.currentHintId.get() === "1_2_conveyor") {
      if (
        currentBuilding &&
        currentBuilding.getId() ===
          gMetaBuildingRegistry.findByClass(MetaBeltBuilding).getId()
      ) {
        // Find closest miner
        const miners = this.root.entityMgr.getAllWithComponent(MinerComponent)

        let closest = null
        const closestDistance = 1e10

        for (let i = 0; i < miners.length; i++) {
          const miner = miners[i]
          const distance =
            miner.components.StaticMapEntity.origin.lengthSquare()
        ...
        if (closest) {
          ...
          const offset =
            staticComp.rotation === 0 ? new Vector(0.5, 0) : new Vector(1, 0.5)
          ...
          const arrowSprite =
            this.root.hud.parts.buildingPlacer.lockIndicatorSprites.regular
    ...
    if (this.currentHintId.get() === "2_2_place_trash") {
      ...
      if (
        currentBuilding &&
        currentBuilding.getId() ===
          gMetaBuildingRegistry.findByClass(MetaTrashBuilding).getId()
      ) {
        const entities = this.root.entityMgr.getAllWithComponent(
          ItemProcessorComponent
        )
        for (let i = 0; i < entities.length; i++) {
          const entity = entities[i]
          if (
            entity.components.ItemProcessor.type !==
