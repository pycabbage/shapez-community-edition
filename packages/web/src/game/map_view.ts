...
export class MapView extends BaseMap {
  ...
  drawStaticEntityDebugOverlays(drawParameters) {
    if (
      G_IS_DEV &&
      (globalConfig.debug.showAcceptorEjectors ||
        globalConfig.debug.showEntityBounds)
    ) {
  ...
  internalInitializeCachedBackgroundCanvases() {
    for (const key in this.cachedBackgroundCanvases) {
      ...
      context.fillStyle =
        THEME.map[`grid${key[0].toUpperCase()}${key.substring(1)}`] || "red"
  ...
  drawForeground(parameters) {
    this.drawVisibleChunks(
      parameters,
      MapChunkView.prototype.drawForegroundDynamicLayer
    )
    this.drawVisibleChunks(
      parameters,
      MapChunkView.prototype.drawForegroundStaticLayer
    )
  }
  ...
  drawVisibleChunks(parameters, method) {
    const cullRange = parameters.visibleRect.allScaled(
  ...
  drawVisibleAggregates(parameters, method) {
    ...
    const aggregateTiles =
      globalConfig.chunkAggregateSize * globalConfig.mapChunkSize
  ...
  drawWiresForegroundLayer(parameters) {
    this.drawVisibleChunks(
      parameters,
      MapChunkView.prototype.drawWiresForegroundLayer
    )
  }
  ...
  drawOverlay(parameters) {
    this.drawVisibleAggregates(
      parameters,
      MapChunkAggregate.prototype.drawOverlay
    )
  }
  ...
  drawBackground(parameters) {
    ...
    this.drawVisibleChunks(
      parameters,
      MapChunkView.prototype.drawBackgroundLayer
    )
  }
}
