...
const overlayMatrices = {
  [defaultBuildingVariant]: generateMatrixRotations([
    0, 1, 1, 1, 1, 0, 0, 1, 1,
  ]),
  [enumRotatorVariants.ccw]: generateMatrixRotations([
    1, 1, 0, 0, 1, 1, 1, 1, 0,
  ]),
  [enumRotatorVariants.rotate180]: generateMatrixRotations([
    1, 1, 0, 1, 1, 1, 0, 1, 1,
  ]),
}
    ...
    switch (variant) {
      case defaultBuildingVariant: {
        const speed = root.hubGoals.getProcessorBaseSpeed(
          enumItemProcessorTypes.rotator
        )
        return [
          [
            T.ingame.buildingPlacement.infoTexts.speed,
            formatItemsPerSecond(speed),
          ],
        ]
      }
      case enumRotatorVariants.ccw: {
        const speed = root.hubGoals.getProcessorBaseSpeed(
          enumItemProcessorTypes.rotatorCCW
        )
        return [
          [
            T.ingame.buildingPlacement.infoTexts.speed,
      ...
      case enumRotatorVariants.rotate180: {
        const speed = root.hubGoals.getProcessorBaseSpeed(
          enumItemProcessorTypes.rotator180
        )
        return [
          [
            T.ingame.buildingPlacement.infoTexts.speed,
