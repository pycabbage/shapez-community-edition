...
} from "../components/underground_belt"
...
export class MetaUndergroundBeltBuilding extends MetaBuilding {
  ...
  getAdditionalStatistics(root, variant) {
    const rangeTiles =
      globalConfig.undergroundBeltMaxTilesByTier[
        enumUndergroundBeltVariantToTier[variant]
      ]
    ...
    const stats = [
      [
        ...
        T.ingame.buildingPlacement.infoTexts.tiles.replace(
          "<x>",
          `${rangeTiles}`
        ),
      ],
    ...
    stats.push([
      ...
      formatItemsPerSecond(beltSpeed),
    ])
    ...
    switch (arrayUndergroundRotationVariantToMode[rotationVariant]) {
      case enumUndergroundBeltMode.sender:
        return Loader.getSprite(
          `sprites/buildings/underground_belt_entry${suffix}.png`
        )
      case enumUndergroundBeltMode.receiver:
        return Loader.getSprite(
          `sprites/buildings/underground_belt_exit${suffix}.png`
        )
      default:
  ...
  getBlueprintSprite(rotationVariant, variant) {
    ...
    switch (arrayUndergroundRotationVariantToMode[rotationVariant]) {
      case enumUndergroundBeltMode.sender:
        return Loader.getSprite(
          `sprites/blueprints/underground_belt_entry${suffix}.png`
        )
      case enumUndergroundBeltMode.receiver:
        return Loader.getSprite(
          `sprites/blueprints/underground_belt_exit${suffix}.png`
        )
      default:
        assertAlways(false, "Invalid rotation variant")
    }
    ...
    for (
      ...
      if (contents) {
        ...
        if (undergroundComp && undergroundComp.tier === tier) {
          ...
          if (staticComp.rotation === targetSenderRotation) {
  ...
  updateVariants(entity, rotationVariant, variant) {
    entity.components.UndergroundBelt.tier =
      enumUndergroundBeltVariantToTier[variant]
    ...
    switch (arrayUndergroundRotationVariantToMode[rotationVariant]) {
      ...
      case enumUndergroundBeltMode.receiver: {
        entity.components.UndergroundBelt.mode =
          enumUndergroundBeltMode.receiver
        entity.components.ItemAcceptor.setSlots([])
        entity.components.ItemEjector.setSlots([
          {
