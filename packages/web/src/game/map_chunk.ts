...
export class MapChunk {
  ...
  internalGeneratePatch(
    ...
    let patchX = rng.nextIntRange(
      border,
      globalConfig.mapChunkSize - border - 1
    )
    let patchY = rng.nextIntRange(
      border,
      globalConfig.mapChunkSize - border - 1
    )
    ...
    for (let i = 0; i <= numCircles; ++i) {
      ...
      const circleX =
        patchX + rng.nextIntRange(-circleOffsetRadius, circleOffsetRadius)
      const circleY =
        patchY + rng.nextIntRange(-circleOffsetRadius, circleOffsetRadius)

      for (
        let dx = -circleRadius * circleScaleX - 2;
        dx <= circleRadius * circleScaleX + 2;
        ++dx
      ) {
        for (
          let dy = -circleRadius * circleScaleY - 2;
          dy <= circleRadius * circleScaleY + 2;
          ++dy
        ) {
  ...
  internalGenerateColorPatch(rng, colorPatchSize, distanceToOriginInChunks) {
    ...
    const availableColors = [enumColors.red, enumColors.green]
  ...
  internalGenerateShapePatch(rng, shapePatchSize, distanceToOriginInChunks) {
    ...
    weights = {
      ...
      [enumSubShape.circle]: Math.round(
        50 + clamp(distanceToOriginInChunks * 2, 0, 50)
      ),
      [enumSubShape.star]: Math.round(
        20 + clamp(distanceToOriginInChunks, 0, 30)
      ),
      [enumSubShape.windmill]: Math.round(
        6 + clamp(distanceToOriginInChunks / 2, 0, 20)
      ),
    }

    for (const key in MODS_ADDITIONAL_SHAPE_MAP_WEIGHTS) {
      weights[key] = MODS_ADDITIONAL_SHAPE_MAP_WEIGHTS[key](
  ...
  generateLowerLayer() {
    const rng = new RandomNumberGenerator(
      `${this.x}|${this.y}|${this.root.map.seed}`
    )
  ...
  generatePatches({ rng, chunkCenter, distanceToOriginInChunks }) {
    ...
    const colorPatchChance =
      0.9 - clamp(distanceToOriginInChunks / 25, 0, 1) * 0.5

    if (rng.next() < colorPatchChance / 4) {
      const colorPatchSize = Math.max(
        2,
        Math.round(1 + clamp(distanceToOriginInChunks / 8, 0, 4))
      )
      this.internalGenerateColorPatch(
        rng,
        colorPatchSize,
        distanceToOriginInChunks
      )
    ...
    const shapePatchChance =
      0.9 - clamp(distanceToOriginInChunks / 25, 0, 1) * 0.5
    if (rng.next() < shapePatchChance / 4) {
      const shapePatchSize = Math.max(
        2,
        Math.round(1 + clamp(distanceToOriginInChunks / 8, 0, 4))
      )
      this.internalGenerateShapePatch(
        rng,
        shapePatchSize,
        distanceToOriginInChunks
      )
  ...
  setLayerContentFromWorldCords(tileX, tileY, contents, layer) {
    ...
    assert(
      contents === null || !oldContents,
      `Tile already used: ${tileX} / ${tileY}`
    )
