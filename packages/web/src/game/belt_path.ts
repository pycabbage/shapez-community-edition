...
export class BeltPath extends BasicSerializableObject {
  ...
  static fromSerialized(root, data) {
    ...
    const fakeObject = /** @type {BeltPath} */ (
      Object.create(BeltPath.prototype)
    )
  ...
  tryAcceptItem(item) {
    if (this.spacingToFirstItem >= globalConfig.itemSpacingOnBelts) {
      ...
      const maxProgress = Math.max(
        0,
        this.spacingToFirstItem - globalConfig.itemSpacingOnBelts
      )
  ...
  computeBounds() {
    let bounds =
      this.entityPath[0].components.StaticMapEntity.getTileSpaceBounds()
  ...
  computeAcceptingEntityAndSlot(debug_Silent = false) {
    ...
    const ejectSlotWsDirection = lastStatic.localDirectionToWorld(
      lastBeltComp.direction
    )
    const ejectSlotWsDirectionVector =
      enumDirectionToVector[ejectSlotWsDirection]
    const ejectSlotTargetWsTile = ejectSlotWsTile.add(
    ...
    const noSimplifiedBelts =
      !this.root.app.settings.getAllSettings().simplifiedBelts

    DEBUG &&
      !debug_Silent &&
      logger.log("  Found target entity", targetEntity.uid)
    ...
    const passOver = this.computePassOverFunctionWithoutBelts(
      targetEntity,
      matchingSlotIndex
    )
    if (!passOver) {
    ...
    return {
      entity: targetEntity,
      acceptor: (item, remainingProgress = 0.0) => {
  ...
  computePassOverFunctionWithoutBelts(entity, matchingSlotIndex) {
    ...
    if (itemProcessorComp) {
      ...
      return (item) => {
        ...
        if (
          !systems.itemProcessor.checkRequirements(
            entity,
            item,
            matchingSlotIndex
          )
  ...
  debug_failIntegrity(change, ...reason) {
    throw new Error(
      `belt path invalid (${change}): ${reason.map((i) => `${i}`).join(" ")}`
    )
  }
  ...
  debug_checkIntegrity(currentChange = "change") {
    ...
    for (let i = 0; i < this.entityPath.length - 1; ++i) {
      ...
      const followUp =
        this.root.systemMgr.systems.belt.findFollowUpEntity(entity)
      if (!followUp) {
        return fail(
    ...
    if (
      this.items.length === 0 &&
      !epsilonCompare(this.spacingToFirstItem, this.totalLength, 0.01)
    ) {
      return fail(
        currentChange,
    ...
    for (let i = 0; i < this.items.length; ++i) {
      ...
      if (
        item[0 /* nextDistance */] < 0 ||
        item[0 /* nextDistance */] > this.totalLength + 0.02
      ) {
        return fail(
    ...
    if (!!acceptor !== !!this.boundAcceptor) {
      return fail(
        "Acceptor target mismatch, acceptor",
        !!acceptor,
        "vs stored",
        !!this.boundAcceptor
      )
    ...
    for (let i = this.items.length - 2; i >= 0; --i) {
      if (
        this.items[i][0 /* nextDistance */] <
        globalConfig.itemSpacingOnBelts + 1e-5
      ) {
        ++firstNonzero
      } else {
        break
  ...
  extendOnEnd(entity) {
    DEBUG &&
      logger.log(
        "Extending belt path by entity at",
  ...
  deleteEntityOnPathSplitIntoTwo(entity) {
    ...
    assert(
      this.entityPath.indexOf(entity) >= 0,
      "Entity not contained for split"
    )
    assert(this.entityPath.indexOf(entity) !== 0, "Entity is first")
    assert(
      this.entityPath.indexOf(entity) !== this.entityPath.length - 1,
      "Entity is last"
    )
    ...
    for (let i = 0; i < this.items.length; ++i) {
      ...
      if (itemPos >= firstPathLength) {
        ...
        DEBUG &&
          logger.log(
            "     Removed item from first path since its no longer contained @",
            itemPos
          )
        ...
        if (itemPos >= secondPathStart) {
          ...
          if (secondPath.items.length === 1) {
            DEBUG &&
              logger.log(
                "       Sinc it was the first, set sapcing of first to",
                itemPos
              )
          ...
          DEBUG &&
            logger.log(
              "    Item was on the removed belt, so its gone - forever!"
            )
        }
      } else {
        ...
        const clampedDistanceToNext =
          Math.min(itemPos + distanceToNext, firstPathLength) - itemPos
  ...
  deleteEntityOnEnd(entity) {
    ...
    if (this.items.length === 0) {
      ...
      if (this.items.length > 0) {
        ...
        assert(
          lastDistance >= 0.0,
          `Last item distance mismatch: ${lastDistance} -> Total length was ${this.totalLength} and lastItemOffset was ${lastItemOffset}`
        )
        ...
        DEBUG &&
          logger.log(
            "  Removed all items so we'll update spacing to total length"
          )
    ...
    if (this.items.length === 0) {
      ...
      if (this.spacingToFirstItem >= beltLength) {
        ...
        if (this.items.length > 0) {
          ...
          DEBUG &&
            logger.log(
              "  We dropped all items, simply set spacing to total length"
            )
  ...
  update() {
    ...
    for (
      lastItemProcessed = this.items.length - 1;
      lastItemProcessed >= 0;
      --lastItemProcessed
    ) {
      ...
      let clampedProgress =
        nextDistanceAndItem[0 /* nextDistance */] - minimumSpacing
      ...
      if (
        ...
        nextDistanceAndItem[0 /* nextDistance */] < 1e-7
      ) {
    ...
    if (lastItem && lastItem[0 /* nextDistance */] === 0) {
      if (this.boundAcceptor?.(lastItem[1 /* item */])) {
        this.items.pop()
        this.numCompressedItemsAfterFirstItem = Math.max(
          0,
          this.numCompressedItemsAfterFirstItem - 1
  ...
  computePositionFromProgress(progress) {
    ...
    for (let i = 0; i < this.entityPath.length; ++i) {
      ...
      if (
        currentLength + localLength >= progress ||
        i === this.entityPath.length - 1
      ) {
        ...
        return this.entityPath[i].components.StaticMapEntity.localTileToWorld(
          localSpace
        )
      }
  ...
  drawDebug(parameters) {
    ...
    for (let i = 0; i < this.items.length; ++i) {
      ...
      const worldPos =
        this.computePositionFromProgress(progress).toWorldSpaceCenterOfTile()
    ...
    for (
      let progress = 0;
      progress <= this.totalLength + 0.01;
      progress += 0.2
    ) {
      const worldPos =
        this.computePositionFromProgress(progress).toWorldSpaceCenterOfTile()
    ...
    parameters.context.fillRect(
      firstItemIndicator.x - 3,
      firstItemIndicator.y - 1,
      6,
      2
    )
  }
  ...
  draw(parameters) {
    ...
    if (this.checkIsPotatoMode()) {
      ...
      if (this.entityPath.length > 1 && firstItem) {
        ...
        const centerPosLocal = beltComp.transformBeltToLocalSpace(
          this.entityPath.length % 2 === 0
            ? beltComp.getEffectiveLengthTiles()
            : 0.5
        )
        const centerPos = staticComp
          .localTileToWorld(centerPosLocal)
          .toWorldSpaceCenterOfTile()

        parameters.context.globalAlpha = 0.5
        firstItem[1 /* item */].drawItemCenteredClipped(
          centerPos.x,
          centerPos.y,
          parameters
        )
    ...
    for (let i = 0; i < this.entityPath.length; ++i) {
      ...
      while (trackPos + beltLength >= currentItemPos - 1e-5) {
        ...
        const localPos = beltComp.transformBeltToLocalSpace(
          currentItemPos - trackPos
        )
        const worldPos = staticComp
          .localTileToWorld(localPos)
          .toWorldSpaceCenterOfTile()
        ...
        if (
          ...
          if (drawStack.length > 1) {
            ...
            if (
              ...
              Math.abs(
                referenceItem[0][drawStackProp] - worldPos[drawStackProp]
              ) < 0.001
            ) {
              // Will continue stack
            } else {
  ...
  drawShapesInARow(
    ...
    h,
    dpi,
    { direction, stack, root, zoomLevel }
  ) {
    ...
    for (let i = 0; i < stack.length; i++) {
      item[1].drawItemCenteredClipped(
        pos.x,
        pos.y,
        parameters,
        globalConfig.defaultItemDiameter
  ...
  drawDrawStack(stack, parameters, directionProp) {
    ...
    const sprite = this.root.buffers.getForKey({
      key: "beltpaths",
      subKey: `stack-${directionProp}-${dpi}#${stack.length}#${firstItem[1].getItemType()}#${firstItem[1].serialize()}`,
      dpi,
      w: dimensions.x,
      h: dimensions.y,
    ...
    const anchor =
      directionVector[inverseDirection] < 0
        ? firstItem
