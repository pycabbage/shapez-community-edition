...
export class PuzzlePlayGameMode extends PuzzleGameMode {
  ...
  constructor(root, { puzzle, nextPuzzles }) {
    ...
    this.additionalHudParts.puzzleCompleteNotification =
  ...
  loadPuzzle() {
    ...
    try {
      ...
      errorText = new PuzzleSerializer().deserializePuzzle(
        this.root,
        this.puzzle.game
      )
    } catch (ex) {
    ...
    const { optionSelected } = this.root.hud.parts.dialogs.showOptionChooser(
      T.dialogs.puzzleReport.title,
      {
        options: [
          ...
          {
            value: "unsolvable",
            text: T.dialogs.puzzleReport.options.unsolvable,
          },
    ...
    return new /** @type {typeof Promise<void>} */ (Promise)((resolve) => {
      optionSelected.add((option) => {
        ...
        this.root.app.clientApi
          .apiReportPuzzle(this.puzzle.meta.id, option)
          .then(
            () => {
              closeLoading()
