...
export class GameSystemWithFilter extends GameSystem {
  ...
  constructor(root, requiredComponents) {
    ...
    this.requiredComponentIds = requiredComponents.map((component) =>
      component.getId()
    )
    ...
    this.root.signals.entityGotNewComponent.add(
      this.internalReconsiderEntityToAdd,
      this
    )
    this.root.signals.entityComponentRemoved.add(
      this.internalCheckEntityAfterComponentRemoval,
      this
    )
    this.root.signals.entityQueuedForDestroy.add(
      this.internalPopEntityIfMatching,
      this
    )
  ...
  internalPushEntityIfMatching(entity) {
    ...
    if (G_IS_DEV && !globalConfig.debug.disableSlowAsserts) {
      assert(
        this.allEntities.indexOf(entity) < 0,
        `entity already in list: ${entity}`
      )
    }
