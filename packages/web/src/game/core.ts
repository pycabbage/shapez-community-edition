...
export class GameCore {
  ...
  initializeRoot(parentState, savegame, gameModeId) {
    ...
    root.keyMapper = new KeyActionMapper(
      root,
      this.root.gameState.inputReceiver
    )

    // Init game mode
    root.gameMode = GameMode.create(
      root,
      gameModeId,
      parentState.creationPayload.gameModeParameters
    )
    ...
    if (G_IS_DEV && globalConfig.debug.manualTickOnly) {
      this.root.gameState.inputReceiver.keydown.add((key) => {
        if (key.keyCode === 84) {
          ...
          // Perform logic ticks
          this.root.time.performTicks(
            this.root.dynamicTickrate.deltaMs,
  ...
  initNewGame() {
    ...
    // Place the hub
    const hub = gMetaBuildingRegistry
    ...
    this.root.camera.center = new Vector(-5, 2).multiplyScalar(
  ...
  initExistingGame() {
    ...
    try {
      const status = serializer.deserialize(
        this.root.savegame.getCurrentDump(),
        this.root
      )
      if (!status.isGood()) {
  ...
  internalInitCanvas() {
    ...
    canvas.classList.toggle(
      "unsmoothed",
      !globalConfig.smoothing.smoothMainCanvas
    )
  ...
  draw() {
    ...
    const effectiveZoomLevel =
      (zoomLevel / globalConfig.assetsDpi) *
      getDeviceDPI() *
      globalConfig.assetsSharpness
    ...
    if (
      this.root.entityMgr.entities.length > 5000 ||
      this.root.dynamicTickrate.averageFps < 50
    ) {
    ...
    assert(
      ...
      "Global alpha not 1 on frame end before restore"
    )
    ...
    params.visibleRect = new Rectangle(
      0,
      0,
      this.root.gameWidth,
    ...
    assert(
      context.globalAlpha === 1.0,
      "context.globalAlpha not 1 on frame end"
    )
    ...
    if (G_IS_DEV && globalConfig.debug.showAtlasInfo) {
      ...
      context.fillText(
        `Atlas: ${desiredAtlasScale} / Zoom: ${round2Digits(zoomLevel)} / Effective Zoom: ${round2Digits(effectiveZoomLevel)}`,
        20,
        600
      )
      ...
      context.fillText(
        `Maintained Buffers: ${stats.rootKeys} root keys / ${stats.subKeys} buffers / VRAM: ${round2Digits(stats.vramBytes / (1024 * 1024))} MB`,
        20,
        620
      )
      const internalStats = getBufferStats()
      context.fillText(
        `Total Buffers: ${internalStats.bufferCount} buffers / ${internalStats.backlogSize} backlog / ${internalStats.backlogKeys} keys in backlog / VRAM ${round2Digits(internalStats.vramUsage / (1024 * 1024))} MB / Backlog ${round2Digits(internalStats.backlogVramUsage / (1024 * 1024))} MB / Created ${internalStats.numCreated} / Reused ${internalStats.numReused}`,
        20,
        640
      )
    }
    ...
    if (G_IS_DEV && globalConfig.debug.testClipping) {
      ...
      context.rect(
        200,
        200,
        this.root.gameWidth - 400,
        this.root.gameHeight - 400
      )
      context.stroke()
    }
  }
