...
export class EntityManager extends BasicSerializableObject {
  ...
  attachDynamicComponent(entity, component) {
    ...
    const componentId = /** @type {typeof Component} */ (
  ...
  destroyEntity(entity) {
    ...
    if (entity.queuedForDestroy) {
      logger.error(
        "Trying to destroy entity which is already queued for destroy!",
        entity.uid
      )
      return
    }
