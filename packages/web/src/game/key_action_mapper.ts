...
export class Keybinding {
  ...
  constructor(
    keyMapper,
    app,
    { keyCode, builtin = false, repeated = false, modifiers = {} }
  ) {
...
export class KeyActionMapper {
  ...
  constructor(root, inputReceiver) {
    ...
    for (const category in KEYMAPPINGS) {
      for (const key in KEYMAPPINGS[category]) {
        const payload = Object.assign({}, KEYMAPPINGS[category][key])
        if (overrides[key]) {
  ...
  forward(receiver, bindings) {
    for (let i = 0; i < bindings.length; ++i) {
      ...
      this.keybindings[key].signal.add((...args) =>
        receiver.keybindings[key].signal.dispatch(...args)
      )
    }
  }
