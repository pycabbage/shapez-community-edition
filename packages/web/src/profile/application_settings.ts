...
function initializeSettings() {
  return [
    ...
    new EnumSetting("autosaveInterval", {
      ...
      textGetter: (interval) =>
        T.settings.labels.autosaveInterval.intervals[interval.id],
    ...
    new EnumSetting("scrollWheelSensitivity", {
      ...
      valueGetter: (scale) => scale.id,
      textGetter: (scale) =>
        T.settings.labels.scrollWheelSensitivity.sensitivity[scale.id],
      category: enumCategories.advanced,
      restartRequired: false,
      changeCb:
        /**
    ...
    new EnumSetting("movementSpeed", {
      ...
      textGetter: (multiplier) =>
        T.settings.labels.movementSpeed.speeds[multiplier.id],
      category: enumCategories.advanced,
      restartRequired: false,
      changeCb: (app, id) => {},
    }),
...
export class ApplicationSettings extends ReadWriteProxy {
  ...
  getSetting(key) {
    assert(
      this.getAllSettings().hasOwnProperty(key),
  ...
  getDesiredFps() {
    return Number.parseInt(this.getAllSettings().refreshRate)
  }
  ...
  verify(data) {
    ...
    if (!THEMES[data.settings.theme]) {
      console.log(
        `Resetting theme because its no longer available: ${data.settings.theme}`
      )
    ...
    for (let i = 0; i < this.settingHandles.length; ++i) {
      ...
      if (!setting.validate(storedValue)) {
        return ExplainedResult.bad(
          `Bad setting value for ${setting.id}: ${storedValue} @ settings version ${data.version} (latest is ${this.getCurrentVersion()})`
        )
      }
    }
  ...
  migrate(data) {
    ...
    if (data.version < 25) {
      ...
      // @ts-ignore
      data.settings.musicMuted = undefined
      // @ts-ignore
      data.settings.soundsMuted = undefined
      data.version = 25
    }
