...
export class EnumSetting extends BaseSetting {
  ...
  validate(value) {
    ...
    const availableValues = this.options.map((option) =>
      this.valueGetter(option)
    )
  ...
  syncValueToElement() {
    ...
    let displayText = "???"
    const matchedInstance = this.options.find(
      (data) => this.valueGetter(data) === value
    )
    if (matchedInstance) {
