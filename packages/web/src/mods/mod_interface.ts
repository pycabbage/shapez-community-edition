...
import { T, matchDataRecursive } from "../translations"
...
export class ModInterface {
  ...
  registerNewBuilding({ metaClass, buildingIconBase64 }) {
    const id = new /** @type {new (...args) => ModMetaBuilding} */ (
      metaClass
    )().getId()
    ...
    metaClass.getAllVariantCombinations().forEach((combination) => {
      ...
      const uniqueTypeId =
        buildingIdentifier +
        (rotationVariant === 0 ? "" : `-${rotationVariant}`)
      ...
      if (combination.tutorialImageBase64) {
        this.setBuildingTutorialImage(
          id,
          variant,
          combination.tutorialImageBase64
        )
      }
    })
  ...
  get dialogs() {
    ...
    throw new Error(
      "Tried to access dialogs but current state doesn't support it"
    )
  }
  ...
  addNewBuildingToToolbar({ toolbar, location, metaClass }) {
    const hudElementName =
      toolbar === "wires" ? "HUDWiresToolbar" : "HUDBuildingsToolbar"
    const property =
      location === "secondary" ? "secondaryBuildings" : "primaryBuildings"
